...
function _isSessionActivelyViewedForList(sid) {
  ...
  // 但用户仍在查看页面，此时不应触发 "finished generating" toast
...
function _reconcileActiveSessionIdleStateFromList(serverRows) {
  ...
  // If the frontend believes a stream is active (S.activeStreamId set and
  // LIVE_STREAMS has a matching entry), do NOT reconcile — the server's
  // session list may lag behind the /api/chat/start DB write. Clearing
  // S.activeStreamId/INFLIGHT here would wipe the optimistic UI and
  // trigger a loadSession that briefly empties the message pane (#2nd-turn-flicker).
  const sid=S.session.session_id;
  if (S.activeStreamId && typeof LIVE_STREAMS!=='undefined' && LIVE_STREAMS[sid]) return false;
...
function _markPollingCompletionUnreadTransitions(sessions) {
  ...
  for (const s of sessions) {
    ...
    // 仅当 observedStreaming 是近期（10分钟内）记录的才视为"刚完成"，
    // 避免页面刷新后 localStorage 中的过期状态误触发 "finished generating" 通知
    const completedPersistedObservedStream = Boolean(
      observedStreaming && !isStreaming
      && observedStreaming.observed_at
      && (Date.now() - observedStreaming.observed_at) < 10 * 60 * 1000
    );
    // 过期的 observedStreaming 条目直接清除，不触发通知
    if (observedStreaming && !isStreaming && observedStreaming.observed_at
        && (Date.now() - observedStreaming.observed_at) >= 10 * 60 * 1000) {
      _forgetObservedStreamingSession(sid);
    }
    if (completedObservedStream || completedPersistedObservedStream || completedWithNewMessages) {
      if (!_isSessionActivelyViewedForList(sid)) {
        ...
        // 后台会话完成生成时，显示 toast 通知和浏览器通知
        const sessionTitle = s.title || 'Session';
        if (typeof showToast === 'function') {
          showToast(sessionTitle + ' finished generating', 4000, 'info');
        }
        if (window._notificationsEnabled && typeof sendBrowserNotification === 'function') {
          sendBrowserNotification('Response complete', sessionTitle + ' finished generating', {sid: sid, force: true});
        }
      } else {
...
async function loadSession(sid){
  ...
  // 冻结工作模式 chip 更新：在 _syncWorkModeForSession 完成前，syncTopbar 等异步
  // 触发的 chip 更新会跳过，避免用旧 S.session 调用 _getCurrentWorkMode 显示旧会话模式（横跳主因）。
  if (typeof _switchingSession !== 'undefined') _switchingSession = true;
  ...
  try {
    ...
    if (typeof _switchingSession !== 'undefined') _switchingSession = false;
    return;
  }
  ...
  // 切换会话时同步工作模式（每个会话独立保存工作模式）
  if(typeof _syncWorkModeForSession==='function') _syncWorkModeForSession();
  if(typeof _hydrateTodosFromSession==='function') _hydrateTodosFromSession(S.session);
  ...
  if(INFLIGHT[sid]&&!_inflightHasVisibleLiveState(INFLIGHT[sid])){
    ...
    // 当 activeStreamId 为 null（流可能已在后台完成）时同样需要清理过时的 INFLIGHT，
    // 避免加载会话时显示为空或进入虚假的忙碌状态。
    delete INFLIGHT[sid];
    if(typeof clearInflightState==='function') clearInflightState(sid);
  }
  ...
  if(INFLIGHT[sid]){
    ...
    if(liveTailPrepared){
      const lastUserInBase=[...S.messages].reverse().find(m=>m&&m.role==='user');
      const lastUserInInflight=[...inflightMessages].reverse().find(m=>m&&m.role==='user');
      const sameLastUser=lastUserInBase&&lastUserInInflight
        &&_sameTranscriptMessage(lastUserInBase,lastUserInInflight);
      if(sameLastUser){
    ...
    if(typeof _hydrateTodosFromSession==='function') _hydrateTodosFromSession(S.session);
    // 只有当服务器确认仍有活跃流时才恢复忙碌状态，避免流在后台完成后切换回会话时进入虚假忙碌
    if(activeStreamId){
      S.busy=true;
    ...
    // 多对话并发支持：当切换回有活跃流的会话时，如果 SSE 连接已断开（LIVE_STREAMS[sid] 不存在），
    // 也需要重新连接 SSE，确保后台生成的内容能继续显示。
    if(activeStreamId&&typeof attachLiveStream==='function'&&(INFLIGHT[sid].reattach||(typeof LIVE_STREAMS==='undefined'||!LIVE_STREAMS[sid]))){
    ...
    // 无论 restoreLiveTurnHtmlForSession 是否成功，都同步刷新 thinkingRow，
    // 确保 streaming-indicator 立即显示。restoreLiveTurnHtmlForSession 恢复的
    // liveAssistantTurn 可能不包含 thinkingRow（例如在静默期保存的快照），
    // 所以需要 _appendThinkingFlush 保证指示器存在。
    if(typeof _appendThinkingFlush==='function' && S.activeStreamId){
      _appendThinkingFlush('');
    }
    ...
    loadDir('.');
    // 不直接调用 setBusy(true)，避免重置已运行的流式计时器（导致 elapsed 从 0 重新开始）
    S.busy=true;updateSendBtn();
    if(typeof _ensureStreamingTimer==='function') _ensureStreamingTimer();
    setComposerStatus('');
    ...
    if(activeStreamId){
      ...
      // 启动流式计时器（Phase 2b 路径缺少 setBusy(true)，导致计时器从未启动）
      if(typeof _ensureStreamingTimer==='function') _ensureStreamingTimer();
      loadDir('.');
      updateQueueBadge(sid);
      startApprovalPolling(sid);
  ...
  // 修复：若消息为空且会话非新建会话，补一次 renderMessages 以清除可能滞留的
  // "Loading conversation..." 占位符。此前当 _ensureMessagesLoaded 静默失败时，
  // 占位符会永久滞留，用户必须手动刷新浏览器才能恢复。
  if (S.messages.length === 0 && S.session && S.session.message_count > 0) {
    renderMessages();
  }
...
async function _checkAndShowHandoffHint(sid) {
  try {
    ...
    if (since != null) body.since = Math.floor(since);
...
async function _ensureMessagesLoaded(sid) {
  ...
  // 改为抛异常而非静默返回，让调用者的 try/catch 能捕获并显示错误 UI，
  // 避免 "Loading conversation..." 占位符永久滞留。
  if (!data || !data.session) {
    throw new Error('MESSAGE_LOAD_EMPTY');
  }
...
function _sameTranscriptMessage(a,b){
  ...
  if(role==='assistant'){
    // 流式过程中，INFLIGHT 里的 assistant 消息可能是部分文本（尚未完成），
    // 与 base 中的完整消息文本不完全一致。去掉空白后比较，或检查前缀关系，
    // 避免切换会话时重复添加部分文本 vs 完整文本的 assistant 消息。
    const aCompact=aText.replace(/\s+/g,'');
    const bCompact=bText.replace(/\s+/g,'');
    if(aCompact===bCompact) return true;
    if(aCompact&&bCompact){
      const shorter=aCompact.length<bCompact.length?aCompact:bCompact;
      const longer=aCompact.length<bCompact.length?bCompact:aCompact;
...
function _dropCurrentTurnAssistantMessages(messages){
  ...
  return list.filter((msg,idx)=>idx<=start||!(msg&&msg.role==='assistant'&&msg._live));
}
...
function _mergeInflightTailMessages(baseMessages, inflightMessages){
  ...
  for(const msg of tail){
    ...
    // 检查整个 merged 数组而非仅尾部窗口：base 中靠前的消息也可能与
    // inflight tail 重复（例如压缩后 base 变短，重复消息不在尾部窗口内）。
    const duplicate=merged.some(existing=>_sameTranscriptMessage(existing,candidate));
...
async function _loadOlderMessages() {
  ...
  _loadingOlder = true;
  // Show the loading state on the "Load earlier messages" indicator immediately.
...
function selectAllSessions(){
  _selectedSessions.clear();
  // 使用 _allSessions（后端返回的完整列表），确保选中所有会话
  // _sessionVisibleSidebarIds 和 _allRenderedSessions 都会遗漏部分会话：
  //   - 折叠分组中的会话（不在 _sessionVisibleSidebarIds 中）
  //   - 无消息的会话（被 _sidebarRowHasVisibleMessages 过滤，不在 _allRenderedSessions 中）
  // 这些被遗漏的会话在删除后会保留，导致"全选后总是留一个"的 bug
  if(Array.isArray(_allSessions)&&_allSessions.length){
    _allSessions.forEach(s=>{
      if(!s||!s.session_id) return;
      // 跳过只读会话（与 checkbox 渲染条件一致：if(_sessionSelectMode&&!readOnly)）
      if(_isReadOnlySession(s)) return;
      _selectedSessions.add(s.session_id);
      // 选中非只读的 fork 子会话
      if(Array.isArray(s._child_sessions)){
        s._child_sessions.forEach(child=>{
          if(child&&child.session_source==='fork'&&child.session_id&&!_isReadOnlySession(child)){
            _selectedSessions.add(child.session_id);
          }
        });
      }
    });
  } else {
    // 回退：使用 DOM 中的 checkbox
...
function _renderBatchActionBar(){
  ...
  deleteBtn.onclick=async()=>{
    ...
    if(!ok)return;
    // Optimistically remove selected sessions from local list for immediate UI feedback
    ids.forEach(sid=>{
    ...
    try{
      ...
      if(S.session&&ids.includes(S.session.session_id)){
        S.session=null;S.messages=[];S.entries=[];localStorage.removeItem('voldp-ai-agent-session');
      ...
      ids.forEach(sid=>_optimisticallyRemovedSessionIds.delete(sid));
    }catch(e){
      ids.forEach(sid=>_optimisticallyRemovedSessionIds.delete(sid));
...
function _mergeOptimisticFirstTurnSessions(fetchedSessions){
  ...
  // 防御性修复：保留不在 fetchedSessions 中的非乐观本地历史会话，防止服务器返回不完整列表时丢失历史会话
  for(const local of Array.isArray(_allSessions)?_allSessions:[]){
    if(!local||!local.session_id) continue;
    if(bySid.has(local.session_id)) continue;
    if(_isOptimisticFirstTurnSessionRow(local)) continue;
    merged.push({...local});
    bySid.set(local.session_id,merged.length-1);
  }
  for(const local of Array.isArray(_allSessions)?_allSessions:[]){
    if(!_isOptimisticFirstTurnSessionRow(local)) continue;
    const sid=local.session_id;
    ...
    if(idx>=0){
      ...
      merged[idx]={
        ...
        message_count:keepLocalOptimistic?Math.max(localCount,fetchedCount):(fetchedCount||localCount),
...
function ensureSessionEventsSSE(){
  ...
  try{
    ...
    _sessionEventsSSE = new EventSource('api/sessions/events', {withCredentials: true});
...
async function probeGatewaySSEStatus(){
  ...
  try{
    // AbortController 超时保护：防止后端误返回 SSE 流导致 fetch 永久挂起
    const _probeCtl = new AbortController();
    const _probeTimer = setTimeout(() => _probeCtl.abort(), 3000);
    let resp;
    try{
      resp = await fetch(new URL('api/sessions/gateway/stream?probe=1', document.baseURI || location.href).href, { credentials:'include', signal: _probeCtl.signal });
    }finally{
...
function startGatewaySSE(){
  ...
  if(_sidebarSseBackgrounded()) return;
  try{
    _gatewaySSE = new EventSource('api/sessions/gateway/stream', {withCredentials: true});
...
function _sessionTitleIsDefaultWebUI(rawTitle){
  ...
  return title==='VoldpAiAgent'||/^VoldpAiAgent #\d+$/.test(title);
}
...
async function _handleActiveSessionStorageEvent(e){
  if(!e || e.key !== 'voldp-ai-agent-session') return;
...
async function deleteSession(sid, beforeDelete=null){
  ...
  if(S.session&&S.session.session_id===sid){
    ...
    if(typeof _hydrateTodosFromSession==='function') _hydrateTodosFromSession(null);
    localStorage.removeItem('voldp-ai-agent-session');
...
function _showProjectContextMenu(e, proj, chip){
  ...
  document.body.appendChild(menu);
  if(typeof _keepMenuInViewport==='function') _keepMenuInViewport(menu,e);
