const TERMINAL_UI={
  ...
  resizeStartY:0,
  resizeStartHeight:0,
  shellOs:'unknown', // 后端操作系统（windows/linux/darwin），决定前端是否手动回显
};
...
function _ensureXterm(){
  ...
  const term=new window.Terminal({
    ...
    scrollback:1000,
    // 管道模式下 cmd.exe 输出可能只含 \n（LF），convertEol:true 让 xterm 自动把 \n 转为 \r\n，
    // 避免光标只下移不回行首导致输出错位。\r\n 中的 \n 不会被重复转换。
    convertEol:true,
  ...
  term.open(surface);
  // 回显策略根据后端操作系统决定：
  // - Windows: cmd.exe /Q 管道模式不逐字符回显，前端必须手动回显提供即时反馈
  // - Linux/macOS: bash -i 会逐字符回显输入，前端不回显避免"输入一个字显示两个字"的重复
  term.onData(data=>{
    const needEcho = TERMINAL_UI.shellOs === 'windows';
    if(needEcho && TERMINAL_UI.term){
      if(data==='\r'){
        TERMINAL_UI.term.write('\r\n');
      }else if(data==='\u007f'||data==='\b'){
        TERMINAL_UI.term.write('\b \b');
      }else if(data==='\u0003'){
        TERMINAL_UI.term.write('^C\r\n');
      }else{
        TERMINAL_UI.term.write(data);
      }
    }
    ...
    if(!sid)return;
    // Windows cmd.exe管道模式需要CRLF执行命令，将CR转换为CRLF
  ...
  // 粘贴说明：xterm.js 内置 paste 支持（textarea 的 paste 事件 → onData）。
  // 不要额外添加 paste 监听器或调用 term.paste()，否则会导致粘贴内容重复两次。
  //
  // Ctrl+V 需要拦截：xterm.js 默认把 Ctrl+V 当作 \x16（literal next 控制字符）输入，
  // 不会触发浏览器 paste 事件。这里返回 false 阻止 xterm.js 处理 Ctrl+V，
  // 让浏览器正常触发 paste 事件，由 xterm.js 内置 paste 处理（只触发一次 onData）。
...
function _connectTerminalOutput(){
  ...
  source.addEventListener('terminal_closed',()=>{
    ...
    if(TERMINAL_UI.term)TERMINAL_UI.term.write('\r\n[terminal closed]\r\n');
  ...
  source.addEventListener('error',()=>{
    console.error('[terminal-debug] SSE 连接错误, readyState=', source.readyState);
  });
}
...
async function _startComposerTerminal(restart=false){
  ...
  try{
    const res=await api('/api/terminal/start',{method:'POST',body:JSON.stringify({
      session_id:sid,
    ...
    if(res&&res.os)TERMINAL_UI.shellOs=res.os;
  }catch(e){
