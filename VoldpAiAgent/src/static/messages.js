...
function _isDocumentVisibleAndFocused() {
  // 仅检查页面可见性，不检查 document.hasFocus()
  // 原因：在 IDE 内置浏览器或分屏环境中，焦点可能在其他窗口/面板上，
  // 但用户仍在查看页面，此时不应误判为"未查看"
...
function _showPersistentStateToast(kind, name, options){
  ...
  if(normalizedKind==='skill'){
    let base;
    if(options&&options.deleted) base=t('skill_deleted');
    else if(options&&options.created) base=t('skill_created');
    else base=t('skill_updated');
...
function _clearStaleBusyStateBeforeSend({compressionRunning=false}={}){
  ...
  if(session) session.pending_started_at=null;
  if(session) session.pending_user_message=null;
...
async function send(){
  ...
  // 如果当前会话已有正在进行的发送，则排队消息；但允许在不同会话中同时发送。
  const _currentViewSid = S.session && S.session.session_id;
  if (_sendInProgress && _sendInProgressSid === _currentViewSid) {
  ...
  // 如果不在流式状态，清理上一轮残留的 #liveAssistantTurn 和 #thinkingRow。
  // 否则 _preservedLiveTurn 会在 renderMessages() 重建后把它们重插入到末尾
  // （在用户新消息之后），导致上一轮 AI 回复出现在新消息下面，顺序错乱。
  // 清除 id 后 _preservedLiveTurn 不会捕获它们，appendThinking() 会创建新的。
  if(!S.activeStreamId){
    const _lt=document.getElementById('liveAssistantTurn');
    if(_lt) _lt.removeAttribute('id');
    const _tr=document.getElementById('thinkingRow');
    if(_tr) _tr.remove();
  }
  let optimisticMessages;
  try{
    S.messages.push(userMsg);
    // Expand the render window so the new user turn doesn't push older
    // context out of view in long conversations (#render-window-context-loss).
    if(typeof _messageRenderWindowSize!=='undefined' && typeof _messageRenderableMessageCount==='function' && typeof _currentMessageRenderWindowSize==='function'){
    ...
    if(typeof saveInflightState==='function'){
      saveInflightState(activeSid,{streamId:null,messages:optimisticMessages,uploaded:uploadedNames,toolCalls:[]});
    }
  ...
  try{
    ...
    const startData=await api('/api/chat/start',{method:'POST',body:JSON.stringify({
      ...
      lang:(typeof _locale!=='undefined'&&_locale._lang)?_locale._lang:'en',
    ...
    if(startData.effective_model && S.session){
      ...
      localStorage.setItem('voldp-ai-agent-model', startData.effective_model);
    ...
    if(e&&e.status===404){
      try{ localStorage.removeItem('voldp-ai-agent-session'); }catch(_){ }
      try{
...
function closeOtherLiveStreams(activeSid){
  // 多对话并发支持：允许后台会话保持SSE连接，但限制最大并发连接数。
  // 浏览器对同一域名的HTTP/1.1连接数有限制（通常6个），
  // 同时还有其他SSE连接（会话事件、审批、澄清等）占用连接池。
  // 为了避免连接池耗尽，限制聊天流SSE连接数为3个。
  // 当超过限制时，关闭最早的非当前会话的聊天流SSE连接。
  const MAX_CONCURRENT_LIVE_STREAMS = 3;
  const sessionIds = Object.keys(LIVE_STREAMS).filter(sid => sid !== activeSid);
  if (sessionIds.length < MAX_CONCURRENT_LIVE_STREAMS) return;
  // 按创建时间排序（LIVE_STREAMS没有存储创建时间，使用INFLIGHT的pending_started_at作为近似值）
  sessionIds.sort((a, b) => {
    const aTime = (INFLIGHT[a] && INFLIGHT[a].startedAt) || 0;
    const bTime = (INFLIGHT[b] && INFLIGHT[b].startedAt) || 0;
    return aTime - bTime;
  });
  // 关闭最早的连接，直到连接数不超过限制
  const toClose = sessionIds.slice(0, sessionIds.length - MAX_CONCURRENT_LIVE_STREAMS + 1);
...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  // 记录流开始时间，用于closeOtherLiveStreams按创建时间排序
  if(!INFLIGHT[activeSid].startedAt) INFLIGHT[activeSid].startedAt=Date.now();
  ...
  function _clearClarifyForOwner(reason){
    // ask_user 工具正在等待用户回复时，流结束/完成事件不应把澄清卡片关掉。
    // 用户提交回答后 respondClarify 会清除 _clarifySessionId，那时再正常隐藏。
    if(_clarifySessionId && _clarifySessionId === activeSid) return;
  ...
  function _finalizeStreamEndFallback(source){
    ...
    if(typeof _clearApiRetryState==='function') _clearApiRetryState();
    if(typeof _clearStreamingFallbackState==='function') _clearStreamingFallbackState();
    ...
    if(_isActiveSession()){
      ...
      if(S.session) S.session.pending_started_at=null;
      if(S.session) S.session.pending_user_message=null;
      // 流结束后总是清除 thinkingRow（streaming-indicator）。
      // 之前用 if(!assistantText) 守卫，导致有内容时 thinkingRow 被 _preservedLiveTurn
      // 保留，streaming-indicator 在流结束后仍显示。
      clearLiveToolCards();removeThinking();
  ...
  function _renderLiveThinking(parsed){
    ...
    // 当没有 reasoning 内容时，不调用 removeThinking。
    // streaming-indicator 应该在整个流式传输期间持续显示（包括切换会话后），
    // done 事件后由 _finalizeStreamEndFallback/removeThinking 移除。
    // 原来的 removeThinking() 会导致切换会话后 streaming-indicator 消失，
    // 即使流仍在运行（特别是纯文本流和 Claude Code 模式，reasoningText 始终为空）。
    // reasoningText 只追加不清空，所以一旦有 reasoning 内容，思考卡片会显示且不会被此处移除。
    // 只在 _showThinking===false 时才移除（已在上面处理）。
  }
  ...
  function _completeAutomaticCompressionOnLiveProgress(sessionId){
    ...
    if(typeof appendLiveCompressionCard==='function'){
      appendLiveCompressionCard({
        ...
        phase:'done',
        automatic:true,
        message:(typeof t==='function'?t('compression_done_message'):'Context auto-compressed'),
      });
    }
    return true;
  }
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('token',e=>{
      ...
      if(typeof _updateStreamingCharCount==='function'&&d.text) _updateStreamingCharCount(d.text.length);
      if(typeof _updateStreamingVerb==='function'&&d.text&&assistantText.length<50) _updateStreamingVerb((typeof t==='function'?t('streaming_responding'):'Responding')+'...');
    ...
    source.addEventListener('tool',e=>{
      ...
      if(typeof _updateStreamingVerb==='function') _updateStreamingVerb((typeof t==='function'?t('streaming_running'):'Running')+'...');
      _completeAutomaticCompressionOnLiveProgress(activeSid);
    ...
    source.addEventListener('state_saved',e=>{
      ...
      if(!S.session||S.session.session_id!==activeSid) return;
      _showPersistentStateToast(d.kind, d.name||'', {
        created:String(d.action||'').toLowerCase()==='created',
        deleted:String(d.action||'').toLowerCase()==='deleted'
      });
    });
    ...
    source.addEventListener('done',e=>{
      ...
      const _finishDone=()=>{
        ...
        if(isActiveSession){
          ...
          S.session=d.session;S.messages=_carryForwardEphemeralTurnFields(S.messages||[], d.session.messages||[]);if(typeof _messagesTruncated!=='undefined')_messagesTruncated=!!d.session._messages_truncated;if(typeof _oldestIdx!=='undefined')_oldestIdx=Number(d.session._messages_offset)||0;
          ...
          if(d.usage){
            ...
            if(S.session){
              for(const _usageField of ['context_length','threshold_tokens','last_prompt_tokens','compression_count']){
          ...
          if(!S.messages.some(m=>m.role==='assistant'&&String(m.content||'').trim())&&!assistantText){S.messages.push({role:'assistant',content:'**No response received.** Check your API key and model selection.'});}
        ...
        // 多对话并发支持：后台会话（非当前视图）完成生成时，强制发送通知，
        // 让用户即使在前台也能收到后台会话完成的通知。
        const isBackgroundSession = !isActiveSession || (S.session && S.session.session_id !== activeSid);
        sendBrowserNotification('Response complete',assistantText?assistantText.slice(0,100):'Task finished',{sid:activeSid, force: isBackgroundSession && window._notificationsEnabled});
      };
    ...
    source.addEventListener('compressing',e=>{
      ...
      const state={
        ...
        phase:'running',
        automatic:true,
        message:(typeof t==='function'?t('compression_running_message'):'Compressing context'),
    ...
    source.addEventListener('compressed',e=>{
      ...
      // 自动压缩后，当前上下文用量归零，压缩次数 +1，方便观察
      if(d.compression_count!=null){
        if(S.session) S.session.compression_count=d.compression_count;
      }else if(S.session){
        S.session.compression_count=(Number(S.session.compression_count)||0)+1;
        d.compression_count=S.session.compression_count;
      }
      // 记录压缩时间点，后续统计 Current 时只考虑压缩之后的新消息
      if(S.session) S.session._lastCompressedAt=Math.floor(Date.now()/1000);
      if(typeof _syncCtxIndicator==='function'){
        if(!d.usage) d.usage={};
        if(d.compression_count!=null) d.usage.compression_count=d.compression_count;
        if(d.usage.last_prompt_tokens==null || d.usage.last_prompt_tokens>0){
          d.usage.last_prompt_tokens=0;
        }
        S.lastUsage=typeof _mergeUsageForCtxIndicator==='function'
    ...
    // 自动重试事件：服务器资源/超时限制导致中断时，后端自动重试并通知前端
    source.addEventListener('retry',e=>{
      if(_terminalStateReached||_streamFinalized) return;
      let d={};
      try{ d=JSON.parse(e.data||'{}')||{}; }catch(_){ d={}; }
      const attempt=d.attempt||1;
      const maxRetries=d.max_retries||3;
      const delayMs=d.delay_ms||0;
      const delaySec=(delayMs/1000).toFixed(1);
      const reason=d.reason||'';
      // 更新流式状态指示器，显示重试提示
      const retryLabel=(typeof t==='function'?t('retry_in_progress'):'Retrying');
      if(typeof _updateStreamingVerb==='function'){
        _updateStreamingVerb(`${retryLabel} (${attempt}/${maxRetries}) ${delaySec}s...`);
      }
      // 记录重试活动到锚点（作为活动边界，不中断已有内容）
      _applyToAnchor('retry',{
        attempt:attempt,
        max_retries:maxRetries,
        delay_ms:delayMs,
        reason:reason,
        status_code:d.status_code||0,
        session_id:d.session_id||activeSid,
      },e);
    });
    ...
    source.addEventListener('apperror',e=>{
      ...
      // 如果后端没有发送session_id，但当前流是活跃流，则认为事件匹配当前会话
      const eventMatchesCurrent=!!(currentSid&&(eventSid===currentSid||continuationSid===currentSid||(eventSid===''&&S.activeStreamId===streamId)));
    ...
    source.addEventListener('warning',e=>{
      ...
      try{
        const d=JSON.parse(e.data);
        // apiRetry / streamingFallback state management
        if(d.type==='api_retry'||d.type==='apiRetry'||d.subtype==='api_retry'){
          if(typeof _setApiRetryState==='function') _setApiRetryState(d);
          // Refresh the thinking row if present so the retry indicator appears
          const row=document.getElementById('thinkingRow');
          if(row&&typeof _renderThinkingInto==='function') _renderThinkingInto(row,'');
          return;
        }
        if(d.type==='streaming_fallback'||d.type==='streamingFallback'||d.subtype==='streaming_fallback'){
          if(typeof _setStreamingFallbackState==='function') _setStreamingFallbackState(d);
          const row=document.getElementById('thinkingRow');
          if(row&&typeof _renderThinkingInto==='function') _renderThinkingInto(row,'');
          return;
        }
    ...
    source.addEventListener('error',async e=>{
      ...
      // 多对话并发支持：即使用户已切换到其他会话，也允许后台会话重连SSE连接。
      // 这样可以确保后台会话的AI回复能够继续接收，切换回会话时能恢复流状态。
      // 重连时检查服务器端的流状态，如果流已结束则恢复消息，如果流还在进行中则重连。
      const _isBackgroundSession = !_isSessionCurrentPane(activeSid);
      // 递归重连函数：支持多次重连（最多5次），每次重连间隔递增（1s、2s、4s、8s、16s）
      async function _attemptReconnect(){
        ...
        _reconnectAttempts++;
        const _delay = Math.min(1000 * Math.pow(2, _reconnectAttempts - 1), 16000);
        if(!_isBackgroundSession) setComposerStatus('Reconnecting…');
        if(_reconnectTimer){ clearTimeout(_reconnectTimer); _reconnectTimer=null; }
        _reconnectTimer=setTimeout(async()=>{
          ...
          try{
            const st=await api(`/api/chat/stream/status?stream_id=${encodeURIComponent(streamId)}`);
            if(st.active){
              if(!_isBackgroundSession) setComposerStatus('Reconnected');
            ...
            if(st.replay_available){
              if(!_isBackgroundSession) setComposerStatus('Restoring stream…');
              _wireSSE(new EventSource(new URL(`api/chat/stream?stream_id=${encodeURIComponent(streamId)}${_runJournalReplayParams()}`,document.baseURI||location.href).href,{withCredentials:true}));
              return;
            }
            // 流已结束，尝试恢复会话消息
...
async function respondApproval(choice) {
  ...
  try {
    ...
    // "本次会话允许"后端已自动启用 YOLO 模式（Rg_XiangYingShenPi2 中 choice=="session" 时设置 Rg_YoloMoShi）。
    // 此处同步前端 _yoloEnabled 状态和 pill UI，避免 UI 与后端不一致。
    if (choice === "session") {
      _yoloEnabled = true;
      _updateYoloPill();
    }
...
// 重连尝试次数与上限。AI 限流场景需要重连,但不能无限重连给服务端施压。
// 上限 30 次 + 指数退避(5s→60s),约 28 分钟后停止;成功连接(initial 事件)后归零。
let _sessionStreamReconnectAttempts = 0;
const _SESSION_STREAM_MAX_RECONNECT = 30;
...
// 延迟重连定时器，避免快速切换可见性时旧连接未完全关闭就创建新连接
let _sessionStreamResumeTimer = null;
...
function startSessionStream(sid) {
  ...
  if (typeof document !== 'undefined' && !document._hermesSessionStreamVisibilityHook) {
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        ...
        // 延迟 500ms 创建新连接，给浏览器足够时间关闭旧连接的 TCP。
        // 避免快速切换可见性时旧连接未完全关闭就创建新连接，
        // 导致同一会话出现 2 个 SSE 连接同时打开（占用连接池槽位）。
        const resumeSid = _sessionStreamHiddenSid;
        _sessionStreamHiddenSid = null;
        if (_sessionStreamResumeTimer) clearTimeout(_sessionStreamResumeTimer);
        _sessionStreamResumeTimer = setTimeout(() => {
          _sessionStreamResumeTimer = null;
  ...
  try {
    const es = new EventSource(_apiUrl('api/session/stream?session_id=' + encodeURIComponent(sid)), {withCredentials: true});
    _sessionEventSource = es;
    es.addEventListener('initial', () => {
      // 连接成功,重置重连计数器
      _sessionStreamReconnectAttempts = 0;
    });
    ...
    es.onerror = () => {
      ...
      // good (readyState === 2) — schedule a one-shot re-open with backoff.
      if (es.readyState === 2 && _sessionStreamSessionId === sid) {
        // 达到重连上限后停止,避免服务端长期不可用时无限重连施压。
        // 上限较高(30 次)且退避可达 60s,足以覆盖 AI 限流场景。
        if (_sessionStreamReconnectAttempts >= _SESSION_STREAM_MAX_RECONNECT) {
        ...
        _sessionStreamReconnectAttempts++;
        // 指数退避:5s,10s,20s,40s,60s,60s... 上限 60s
        const _delay = Math.min(5000 * Math.pow(2, _sessionStreamReconnectAttempts - 1), 60000);
...
function stopSessionStream() {
  ...
  if (_sessionStreamResumeTimer) { clearTimeout(_sessionStreamResumeTimer); _sessionStreamResumeTimer = null; }
...
function showClarifyCard(pending) {
  ...
  // Persist the AI's clarifying question into the transcript immediately so it
  // survives connection drops / page reloads while waiting for the user answer.
  if (S.session && S.session.session_id === sid && question) {
    const last = S.messages[S.messages.length - 1];
    const lastContent = last ? msgContent(last) : "";
    if (!last || last.role !== "assistant" || lastContent !== question) {
      S.messages.push({
        role: "assistant",
        content: question,
        _clarify_question: true,
        _ts: Date.now() / 1000,
      });
  ...
  if (choicesEl) {
    ...
    if (choices.length) {
      choices.forEach((choice, idx) => {
        ...
        btn.onclick = () => _toggleClarifyChoice(btn, choice);
        const indicator = document.createElement('span');
        indicator.className = 'clarify-choice-indicator';
      ...
      const otherIndicator = document.createElement('span');
      otherIndicator.className = 'clarify-choice-indicator';
      const otherText = document.createElement('span');
      otherText.className = 'clarify-choice-text';
      otherText.textContent = t('clarify_other') || '其他';
      other.appendChild(otherIndicator);
      other.appendChild(otherText);
      other.onclick = () => {
        _clearClarifyChoiceSelection();
  ...
  if (input) {
    ...
    input.oninput = () => {
      if (input.value.trim()) _clearClarifyChoiceSelection();
    };
  }
...
function _getClarifySelectedChoice() {
  const choicesEl = $("clarifyChoices");
  if (!choicesEl) return '';
  const selected = choicesEl.querySelector('.clarify-choice.selected:not(.other)');
  return selected ? selected.dataset.choice || '' : '';
}

function _clearClarifyChoiceSelection() {
  const choicesEl = $("clarifyChoices");
  if (!choicesEl) return;
  choicesEl.querySelectorAll('.clarify-choice.selected').forEach((btn) => btn.classList.remove('selected'));
}

function _toggleClarifyChoice(btn, choice) {
...
async function respondClarify(response) {
  ...
  if (!sid) return;
  const input = $("clarifyInput");
  let value = typeof response === 'string' ? response : (_getClarifySelectedChoice() || (input ? input.value : ''));
  ...
  try {
    ...
    if (result && result.ok) {
      ...
      if (_clarifyId === clarifyId) {
        ...
        // and preserve the AI's clarifying question so it stays in history after the card is hidden.
        if (S.session && S.session.session_id === sid) {
          const questionEl = $("clarifyQuestion");
          const questionText = questionEl ? String(questionEl.textContent || "").trim() : "";
          if (questionText) {
            const last = S.messages[S.messages.length - 1];
            const lastContent = last ? msgContent(last) : "";
            if (!last || last.role !== "assistant" || lastContent !== questionText) {
              S.messages.push({
                role: "assistant",
...
function playNotificationSound(){
  ...
  try{
    const ctx=window._getSharedAudioContext?window._getSharedAudioContext():new (window.AudioContext||window.webkitAudioContext)();
    if(!ctx)return;
...
function _notificationOptions(body,options={}){
  ...
  return {body:body||'',tag:sid?`voldp-ai-agent-${sid}`:'voldp-ai-agent',renotify:false,icon:'static/favicon-192.png',badge:'static/favicon-32.png',data:{url}};
}
...
function attachBtwStream(parentSid, streamId, question){
  ...
  const src=new EventSource(new URL('api/chat/stream?stream_id='+encodeURIComponent(streamId), document.baseURI||location.href).href, {withCredentials: true});
  let answer='';
  let btwRow=null;
      ...
      if(r&&r.results){
        for(const res of r.results){
          if(res.task_id===taskId){
            ...
            // 仅当用户仍停留在发起后台任务的会话时，才将结果追加到消息列表
            // 避免切换到其他会话后，后台任务结果污染新会话的消息列表
