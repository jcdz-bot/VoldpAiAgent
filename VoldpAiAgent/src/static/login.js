...
document.addEventListener('DOMContentLoaded', function () {
  // 移除火山编译器插入的 //vl: 行号调试标记，避免显示在页面上
  (function removeVlMarkers(node) {
    if (!node) return;
    if (node.nodeType === Node.TEXT_NODE) {
      var t = String(node.nodeValue || '');
      if (/\/\/vl:\s*\d+/.test(t)) {
        var cleaned = t.replace(/\s*\/\/vl:\s*\d+/g, '').replace(/^\s+|\s+$/g, '');
    ...
    for (var i = 0; i < children.length; i++) removeVlMarkers(children[i]);
  })(document.body);
  ...
  // 解析相对 API 路径为绝对 URL，支持子路径挂载(如 /hermes/)
  ...
  async function doLogin(e) {
    ...
    setLoading(true);
    try {
      var res = await fetch(_apiUrl('api/auth/login'), {
        method: 'POST',
  ...
  async function doPasskeyLogin() {
    ...
    try {
      ...
      var optRes = await fetch(_apiUrl('api/auth/passkey/options'), { method: 'POST', body: '{}', credentials: 'include' });
      ...
      var res = await fetch(_apiUrl('api/auth/passkey/login'), {
  ...
  (function checkConnectivity() {
    ...
    function probe() {
      fetch(_apiUrl('health'), { method: 'GET', credentials: 'same-origin' })
        .then(function (r) {
          if (r.ok) {
