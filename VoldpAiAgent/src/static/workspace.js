async function api(path,opts={}){
  ...
  // 默认 60 秒超时（原 30 秒在连接池紧张时容易误触发，导致"请求失败，正在重试"toast
  // 但实际并未重试）。连接池被多个 SSE 长连接占用时，新请求可能排队等待槽位释放，
  // 60 秒给浏览器足够时间复用连接或等待 SSE 释放。
  const timeoutMs=Object.prototype.hasOwnProperty.call(opts,'timeoutMs')?opts.timeoutMs:60000;
  const timeoutToast=opts.timeoutToast!==false;
  // Retry up to 2 times on network errors (TypeError) AND client-side timeouts.
  // 超时重试是安全的：连接池耗尽时请求尚未真正发送到服务器，重试不会产生副作用。
  ...
  for(let attempt=0;attempt<3;attempt++){
    ...
    try{
      ...
      return useTimeout?await Promise.race([
        ...
        new Promise((_,reject)=>{
          timeoutId=setTimeout(()=>{
            ...
            const msg=typeof t==='function'?t('request_timed_out'):'Request failed, retrying';
      ...
      // 超时也触发重试（连接池耗尽时请求未真正发送，重试安全）
      if(isTimeout){
        // 最后一次尝试才显示 toast 并抛出，前两次静默重试
        if(attempt<2){
          // 静默重试，不显示 toast
        }else{
          const msg=typeof t==='function'?t('request_timed_out'):'Request failed, retrying';
          const err=(e&&e.name==='TimeoutError')?e:new Error(msg);
          err.name='TimeoutError';
          err.timeout=true;
          if(timeoutToast&&typeof showToast==='function') showToast(msg,5000,'error');
          throw err;
        }
        continue;
      }
...
// uploadWithProgress: 基于 XHR 的文件上传，支持实时字节进度回调。
// fetch API 不支持上传进度回调，所以文件上传用 XHR 实现。
// onProgress(loaded, total) 在上传过程中被调用。
// 返回对象: {ok, status, statusText, data(已解析的JSON或null), text(原始响应文本)}
function uploadWithProgress(url, formData, onProgress, opts) {
  opts = opts || {};
  ...
  return new Promise(function(resolve, reject) {
    ...
    // XHR 绕过了 index.html 中对 window.fetch 的 CSRF monkey-patch，需要手动带上 token 头，
    // 否则后端 CSRF 中间件会返回 "CSRF token mismatch"（Linux/Windows 均会触发）。
    try {
      const csrfToken = (window.__HERMES_CONFIG__ || {}).csrfToken || '';
      if (csrfToken) xhr.setRequestHeader('X-Hermes-CSRF-Token', csrfToken);
    } catch (_) {}
    ...
    xhr.ontimeout = function() {
      const msg = typeof t === 'function' ? t('request_timed_out') : 'Request timed out';
...
function _wsExpandKey(){
  ...
  return ws?'voldp-ai-agent-expanded:'+ws:null;
}
...
async function openArtifactPath(path){
  ...
  if(!rel) rel = '.';
  // 绝对路径（Windows X:\ 或 Unix /）直接打开，后端 Rg_QuWenJianJueDuiLuJing
  // 会通过 filepath.IsAbs 正确处理，无需前端 _workspacePathExists 预检
  //（该函数用 '/' 分割路径，Windows 反斜杠路径会误判为不存在）
  const isAbs = /^[A-Za-z]:[\\/]/.test(rel) || rel.startsWith('/');
  if(!isAbs){
    try{
...
async function uploadToWorkspace(file, dir) {
  ...
  try {
    const url = new URL('api/workspace/upload', document.baseURI || location.href).href;
    if (bar && barWrap) { barWrap.classList.add('active'); bar.style.width = '0%'; }
    const res = await uploadWithProgress(url, formData, function(loaded, total) {
      if (bar && total > 0) bar.style.width = Math.round((loaded / total) * 100) + '%';
    }, { timeoutMs: 120000 });
    if (bar && barWrap) { barWrap.classList.remove('active'); bar.style.width = '0%'; }
    // 401 重定向到登录页
    if (res.status === 401) { window.location.href = 'login?next=' + encodeURIComponent(window.location.pathname + window.location.search); return; }
    const data = res.data;
    if (!res.ok || (data && data.error)) {
      showToast((data && data.error) || res.statusText || ('HTTP ' + res.status), 5000, 'error');
