...
// 归档解压端点 /api/upload/extract 后端用 io.LimitReader 截断到 20MB（防解压炸弹），
// 前端必须同步预校验，否则超限会得到模糊的 "Invalid zip file" 错误。
const MAX_ARCHIVE_UPLOAD_BYTES=20*1024*1024;
const MAX_ARCHIVE_UPLOAD_MB=20;
...
let _emptyStateIntroCache={};
let _emptyStateIntroPersonality=null;
async function updateEmptyStateForPersonality(personality){
  const name=(personality&&String(personality).trim())||'helpful';
  const titleEl=$('emptyStateTitle');
  const subEl=$('emptyStateSubtitle');
  if(!titleEl||!subEl) return;
  if(_emptyStateIntroCache[name]){
    const cached=_emptyStateIntroCache[name];
    titleEl.textContent=cached.headline;
    subEl.textContent=cached.body;
    _emptyStateIntroPersonality=name;
    return;
  }
  try{
    const data=await api(`/api/personalities/intro?personality=${encodeURIComponent(name)}`);
    if(data && data.headline){
      _emptyStateIntroCache[name]=data;
      titleEl.textContent=data.headline;
      subEl.textContent=data.body;
      _emptyStateIntroPersonality=name;
    }
  }catch(_){}
}
...
function _wireMessageWindowLoadEarlierButton(){
  ...
  if(!indicator) return;
  indicator.onclick=()=>{
    if(typeof _loadingOlder!=='undefined'&&_loadingOlder) return;
...
async function refreshProviderQuotaIndicator(){
  ...
  try{
    const status=await api('/api/providers/quota');
...
const MODEL_STATE_KEY='voldp-ai-agent-model-state';
const PENDING_SESSION_MODEL_PREFIX='voldp-ai-agent-pending-session-model:';
...
function _readPersistedModelState(){
  ...
  const legacy=localStorage.getItem('voldp-ai-agent-model');
...
function _writePersistedModelState(model, modelProvider){
  ...
  if(!value){
    localStorage.removeItem('voldp-ai-agent-model');
    localStorage.removeItem(MODEL_STATE_KEY);
    return;
  }
  localStorage.setItem('voldp-ai-agent-model', value);
  try{
...
function _persistSessionModelCorrection(model, provider, opts){
  if(!S.session) return;
  let request=fetch(new URL('api/session/update',document.baseURI||location.href).href,{
    ...
    try{
      ...
      if(data&&data.session&&S.session){
        S.session.context_length=data.session.context_length||S.session.context_length||0;
        S.session.threshold_tokens=data.session.threshold_tokens||S.session.threshold_tokens||0;
        if(typeof _syncCtxIndicator==='function'){
          _syncCtxIndicator({
            context_length:S.session.context_length,
            threshold_tokens:S.session.threshold_tokens,
            last_prompt_tokens:S.session.last_prompt_tokens||0,
            compression_count:S.session.compression_count||0,
            input_tokens:S.session.input_tokens||0,
            output_tokens:S.session.output_tokens||0,
          });
        }
      }
    }catch(_){}
...
async function populateModelDropdown(opts={}){
  ...
  if(!sel) return;
  try{
    const _modelsUrl=new URL('api/models',document.baseURI||location.href);
    _modelsUrl.searchParams.set('_t',Date.now());
    const _modelsRes=await fetch(_modelsUrl.href,{credentials:'include'});
    ...
    // Put the configured default model at the top of the list so users can
    // quickly re-select it without hunting through provider groups.
    const defaultModel=data.default_model;
    if(defaultModel){
      const defaultLabel=(typeof t==='function' && t('providers_label_default_model')) || 'Default model';
      groups.unshift({
        provider: defaultLabel,
        provider_id: data.active_provider || 'configured',
        models: [{id: defaultModel, label: (typeof getModelLabel==='function'?getModelLabel(defaultModel):defaultModel)}]
      });
    }
...
// Expose to window so panels.js can clear it on provider changes
 ...
 */
function _checkProviderMismatch(modelId, dropdownModelProvider){
  ...
  // If the dropdown tells us this option belongs to the active provider, trust it
  // (e.g. Nvidia serves models whose ID prefix is the original publisher like moonshotai/...).
  const optProvider=String(dropdownModelProvider||'').toLowerCase();
  if(optProvider && optProvider===ap) return null;
  // 用户明确选择了不同的提供商（如硅基流动、OpenRouter等代理提供商），
  // 信任用户的选择，不显示警告——代理提供商可以路由各种厂商前缀的模型
  if(optProvider && optProvider!==ap) return null;
...
function _formatReasoningEffortLabel(effort){
  if(typeof t==='function'){
    const key='reasoning_effort_'+effort;
...
function _applyReasoningOptions(supportedEfforts){
  ...
  dd.querySelectorAll('.reasoning-btn').forEach(function(opt){
...
document.addEventListener('click',function(e){
  ...
  if(e.target.closest('.reasoning-btn')){
    const opt=e.target.closest('.reasoning-btn');
    ...
    if(effort){
      ...
      api('/api/reasoning',{method:'POST',body:JSON.stringify(payload)})
        .then(function(st){
          ...
          const effLabel=typeof t==='function'?t('reasoning_effort_'+eff):eff;
          showToast('🧠 '+(typeof t==='function'?t('reasoning_effort_set_to',effLabel):('Reasoning effort set to '+eff)));
        })
        .catch(function(){showToast('🧠 '+(typeof t==='function'?t('reasoning_effort_set_failed'):'Failed to set effort'));});
      closeReasoningDropdown();
    }
  }
});

// ── Session toolsets chip (#493) — REMOVED (功能未在 Go 端实现，保留 DB 字段不影响) ──
...
(function(){
  ...
  el.addEventListener('scroll',()=>{
    ...
    _scrollRaf=requestAnimationFrame(()=>{
      ...
      const olderPrefetchPx=Math.min(900,Math.max(360,el.clientHeight));
      const nearHardTop=el.scrollTop<=2;
      if(_isSessionEndlessScrollEnabled()&&!(S.busy||S.activeStreamId)&&typeof _loadingOlder!=='undefined'&&!_loadingOlder&& typeof _messagesTruncated!=='undefined' && _messagesTruncated && typeof _loadOlderMessages==='function'&&(nearHardTop||el.scrollTop<olderPrefetchPx)){
        _loadOlderMessages();
      }
    });
  });
...
function _mergeUsageForCtxIndicator(latest, fallback){
  ...
  // 累积类字段：最新的 0 不应抹掉之前已统计到的正数（例如上下文压缩事件会清零 usage）
  for(const field of ['input_tokens','output_tokens','estimated_cost','compression_count']){
    const latestVal=latestObj[field];
    const fallbackVal=fallbackObj[field];
    if(latestVal!=null && latestVal!==0){
      merged[field]=latestVal;
    }else if(fallbackVal!=null && fallbackVal!==0){
      merged[field]=fallbackVal;
    }else if(merged[field]==null && fallbackVal!=null){
      merged[field]=fallbackVal;
    }
  }
  for(const field of [
  ...
  if(latestObj.compression_count==null&&fallbackObj.compression_count!=null){
    merged.compression_count=fallbackObj.compression_count;
  }
  return merged;
}

// 从最近一次 assistant 消息的真实 usage 中获取当前 prompt tokens；
// 若不可用（如正在用户回合）则按消息字符数估算。压缩后旧消息仍保留，
// 但最新 assistant 的 _turnUsage.input_tokens 反映的是实际发送给模型的 prompt。
function _getMessageTs(m){ return (m&&m._ts)||(m&&m.timestamp)||0; }

function _getLastTurnUsageFromMessages(){
  if(!window.S || !Array.isArray(S.messages)) return null;
  const cutoff=(S.session && S.session._lastCompressedAt) || 0;
  for(let i=S.messages.length-1;i>=0;i--){
    const m=S.messages[i];
    if(!m || m.role!=='assistant') continue;
    // 压缩前的 assistant usage 属于旧上下文，不应再计入当前用量
    if(cutoff>0){
      const ts=_getMessageTs(m);
      if(ts && ts <= cutoff) break;
    }
    const u=m._turnUsage || m.usage;
    if(!u) continue;
    const inTok=u.input_tokens||0;
    const outTok=u.output_tokens||0;
    const cacheRead=u.cache_read_tokens||0;
    const cacheWrite=u.cache_write_tokens||0;
    if(inTok===0 && outTok===0 && cacheRead===0 && cacheWrite===0) continue;
    return {
      input_tokens:inTok,
...
function _estimatePromptTokensFromMessages(){
  const lastUsage=_getLastTurnUsageFromMessages();
  if(lastUsage && lastUsage.input_tokens>0){
    return lastUsage.input_tokens + lastUsage.cache_read_tokens + lastUsage.cache_write_tokens;
  }
  // 无真实 usage 时按压缩后的消息字符数估算
  // 跳过 tool 消息：工具结果属于工具UI组件，不应计入对话上下文统计
  // reasoning 内容存储在 m.reasoning 字段中（与 m.content 分开），不会被计入
  const cutoff=(S.session && S.session._lastCompressedAt) || 0;
  let chars=0;
  for(const m of S.messages){
    if(!m) continue;
    ...
    const text=(typeof m.content==='string'?m.content:(Array.isArray(m.content)?m.content.map(c=>c&&typeof c.text==='string'?c.text:'').join(''):''));
    chars+=text.length;
  }
  if(chars<=0) return 0;
...
function _syncCtxIndicator(usage){
  ...
  // 后端未提供或返回 0 时，按当前消息内容估算实时 prompt tokens
  let promptTok=usage.last_prompt_tokens||0;
  if(promptTok<=0 && S && Array.isArray(S.messages) && S.messages.length>0){
    promptTok=_estimatePromptTokensFromMessages();
  }
  const inTok=usage.input_tokens||(S.session&&S.session.input_tokens)||0;
  const outTok=usage.output_tokens||(S.session&&S.session.output_tokens)||0;
  const totalTok=inTok+outTok;
  const cacheReadTok=usage.cache_read_tokens||0;
  const cacheWriteTok=usage.cache_write_tokens||0;
  const compressCount=Number(usage.compression_count||0);
  // Prefer session's configured context window; fall back to usage or default
  const DEFAULT_CTX=128*1024;
  const ctxWindow=(S.session&&S.session.context_length)||usage.context_length||DEFAULT_CTX;
  if(S.lastUsage && ctxWindow && ctxWindow!==S.lastUsage.context_length){
    S.lastUsage.context_length=ctxWindow;
  }
  const threshold=(S.session&&S.session.threshold_tokens)||usage.threshold_tokens||0;
  if(S.lastUsage && threshold && threshold!==S.lastUsage.threshold_tokens){
    S.lastUsage.threshold_tokens=threshold;
  }
  const cost=usage.estimated_cost;
  // Show indicator whenever we have any usage data (tokens or cost) or compression history
  if(!promptTok&&!totalTok&&!cost&&!cacheReadTok&&!cacheWriteTok&&!compressCount){
  ...
  let label=t('ctx_window_current',_fmtTokens(promptTok),_fmtTokens(ctxWindow));
  if(!hasExplicitCtx) label+=t('ctx_est_128k');
  if(cost) label+=` \u00b7 $${cost<0.01?cost.toFixed(4):cost.toFixed(2)}`;
  if(cacheText) label+=` \u00b7 ${cacheText}`;
  el.setAttribute('aria-label',label);
  const usageText=t('ctx_tokens_used',_fmtTokens(totalTok),_fmtTokens(inTok),_fmtTokens(outTok));
  const inOutText=t('ctx_in_out',_fmtTokens(inTok),_fmtTokens(outTok));
  const tokensText=t('ctx_current',_fmtTokens(promptTok),_fmtTokens(ctxWindow));
  const compressCountText=compressCount===1?t('ctx_compressed_one_time'):t('ctx_compressed_times',compressCount);
  if(usageLine) usageLine.textContent=usageText;
  if(tokensLine) tokensLine.textContent=tokensText;
  // 显示剩余可用token数
  const freeLine=$('ctxTooltipFree');
  if(freeLine){
    if(hasPromptTok&&ctxWindow){
      const freeTok=Math.max(0,ctxWindow-promptTok);
      const freePct=Math.max(0,Math.round((freeTok/ctxWindow)*100));
      freeLine.textContent=t('ctx_free_tokens',_fmtTokens(freeTok),freePct);
      freeLine.style.display='';
    }else{
      freeLine.style.display='none';
    }
  }
  // 显示当前模型名
  const modelLine=$('ctxTooltipModel');
  if(modelLine){
    const modelName=usage.model||S.session&&S.session.model||'';
    if(modelName){
      modelLine.textContent=t('ctx_model',modelName);
      modelLine.style.display='';
    }else{
      modelLine.style.display='none';
    }
  }
  const compressCountLine=$('ctxTooltipCompressCount');
  if(compressCountLine){
    compressCountLine.textContent=compressCountText;
    compressCountLine.style.display='';
  }
  const inOutLine=$('ctxTooltipTokensInOut');
  if(inOutLine) inOutLine.style.display='none';
  let thresholdText='';
  if(thresholdLine){
...
function setBusy(v){
  ...
  updateSendBtn();
  if(v){
    if(typeof _startStreamingTimer==='function') _startStreamingTimer();
  }
  if(!v){
    if(typeof _clearStreamingTimer==='function') _clearStreamingTimer();
...
let _streamingElapsedTimer=null;
let _streamingStartMs=0;
let _streamingCharCount=0;
let _apiRetryState=null;       // {errorType, errorStatus, retryCount, retryDelayMs, receivedAt}
let _streamingRetryTimer=null; // countdown interval for apiRetry
let _streamingFallbackState=null; // {message}

function _startStreamingTimer(){
  _clearStreamingTimer();
  _streamingStartMs=Date.now();
  _streamingCharCount=0;
  _updateStreamingIndicator();
  _streamingElapsedTimer=setInterval(_updateStreamingIndicator,1000);
}

// 确保流式计时器在运行，但不重置已运行计时器的起始时间。
// 用于切换会话时保持已耗时显示（避免 elapsed 时间从 0 重新开始）。
// 当 DOM 中的 streamingElapsed 元素被重新渲染为空时，立即更新一次。
function _ensureStreamingTimer(){
  if(_streamingElapsedTimer){
    _updateStreamingIndicator();
    return;
  }
...
function _clearStreamingTimer(){
  if(_streamingElapsedTimer){clearInterval(_streamingElapsedTimer);_streamingElapsedTimer=null;}
  if(_streamingRetryTimer){clearInterval(_streamingRetryTimer);_streamingRetryTimer=null;}
  _streamingStartMs=0;
  _streamingCharCount=0;
  _apiRetryState=null;
  _streamingFallbackState=null;
}

function _setApiRetryState(data){
  _apiRetryState={
    errorType: data.errorType||data.error_type||'error',
    errorStatus: data.errorStatus||data.error_status||0,
    retryCount: data.retryCount||data.retry_count||1,
    retryDelayMs: data.retryDelayMs||data.retry_delay_ms||5000,
    receivedAt: Date.now(),
    message: data.message||''
  };
  _updateStreamingRetryCountdown();
  if(!_streamingRetryTimer) _streamingRetryTimer=setInterval(_updateStreamingRetryCountdown,1000);
}
function _clearApiRetryState(){
  _apiRetryState=null;
  if(_streamingRetryTimer){clearInterval(_streamingRetryTimer);_streamingRetryTimer=null;}
}
function _setStreamingFallbackState(data){
  _streamingFallbackState={message: data.message||data.text||'Waiting for fallback response...'};
}
function _clearStreamingFallbackState(){
  _streamingFallbackState=null;
}
function _updateStreamingRetryCountdown(){
  const wrap=document.getElementById('streamingIndicator');
  if(!wrap) return;
  const countdownEl=document.getElementById('streamingRetryCountdown');
  if(!countdownEl||!_apiRetryState) return;
  const elapsed=Date.now()-_apiRetryState.receivedAt;
  const remaining=Math.max(0,Math.ceil((_apiRetryState.retryDelayMs-elapsed)/1000));
  countdownEl.textContent=remaining>0?`Retrying in ${remaining}s`:'Retrying now...';
}

function _updateStreamingCharCount(delta){
  _streamingCharCount+=delta;
  _updateStreamingIndicator();
}

function _updateStreamingIndicator(){
  const el=document.getElementById('streamingIndicator');
  if(!el) return;
  const elapsedEl=document.getElementById('streamingElapsed');
  const tokensEl=document.getElementById('streamingTokens');
  if(_streamingStartMs>0&&elapsedEl){
    const sec=Math.floor((Date.now()-_streamingStartMs)/1000);
    if(sec>0){
  ...
  if(tokensEl&&_streamingCharCount>0){
    const tokens=Math.round(_streamingCharCount/4);
    tokensEl.textContent='↓ '+tokens+' tokens';
  }
}

function _updateStreamingVerb(verb){
  const el=document.querySelector('.streaming-indicator-verb');
  if(el) el.textContent=verb;
}

function _syncComposerHeightVar(){
  const wrap=document.getElementById('composerWrap');
  if(!wrap) return;
  document.documentElement.style.setProperty('--composer-height', wrap.offsetHeight+'px');
}
// 同步 composer-box 的水平中心位置到 CSS 变量，用于 thinkingRow.thinking-row-streaming 对齐。
// thinkingRow 使用 position:fixed 相对于视口定位，但 composer-box 在主区域内居中
// （主区域 = 视口 - 侧边栏）。当侧边栏展开或视口被 DevTools 挤压时，两者的中心不一致，
// 导致 streaming-indicator 与输入框不对齐。此函数将 composer-box 的中心像素值写入
// --composer-center 变量，thinkingRow 用它替代 left:50% 实现精确对齐。
function _syncComposerCenterVar(){
  const box=document.querySelector('.composer-box');
  if(!box) return;
  const rect=box.getBoundingClientRect();
  if(!rect.width) return;
  const center=rect.left+rect.width/2;
  document.documentElement.style.setProperty('--composer-center', center+'px');
  // 同步 composer-box 实际渲染宽度，用于约束 thinkingRow 的 max-width。
  // thinkingRow 是 position:fixed，不受 main 区域宽度限制，
  // 需要显式约束否则工作区面板展开时会超出对话区。
  document.documentElement.style.setProperty('--composer-width', rect.width+'px');
}
function _setupComposerHeightObserver(){
  if(!window.ResizeObserver) return;
  const wrap=document.getElementById('composerWrap');
  if(!wrap) return;
  _syncComposerHeightVar();
  _syncComposerCenterVar();
  const ro=new ResizeObserver(()=>{_syncComposerHeightVar();_syncComposerCenterVar();});
  ro.observe(wrap);
  // 监听 main 元素（侧边栏折叠/展开会改变其宽度），以及 composer-box 本身
  const mainEl=document.querySelector('main.main');
  if(mainEl) ro.observe(mainEl);
  const cbox=document.querySelector('.composer-box');
  if(cbox) ro.observe(cbox);
}
_setupComposerHeightObserver();
// 窗口缩放（如按 F12 打开 DevTools）时同步更新
window.addEventListener('resize', _syncComposerCenterVar, {passive:true});
// 侧边栏折叠/展开有 0.24s 过渡动画，期间持续更新位置
(function _setupSidebarTransitionSync(){
  ...
  // 监听 .layout 的 class 变化（toggleSidebar 会切换 sidebar-collapsed 类）
  const layout=document.querySelector('.layout');
  if(layout){
    const mo=new MutationObserver(muts=>{
      for(const m of muts){
        if(m.type==='attributes'&&m.attributeName==='class'){startTick();break;}
      }
    });
  ...
  if(sidebar){
    sidebar.addEventListener('transitionend',e=>{
      if(e.propertyName==='width') _syncComposerCenterVar();
    });
  }
  // 工作区面板（.rightpanel）的 width 过渡也需要兜底
...
if(document.readyState==='loading'){
  document.addEventListener('DOMContentLoaded', _setupComposerHeightObserver);
}
...
function _ensureAppDialogBindings(){
  ...
  if(confirmBtn){
    confirmBtn.addEventListener('click',()=>{
      ...
      }else if(APP_DIALOG.kind==='folderpicker'){
        _finishAppDialog(APP_DIALOG.selectedValue!=null?APP_DIALOG.selectedValue:(APP_DIALOG.currentPath||''));
      }else{
...
function showPromptDialog(opts={}){
  ...
  const overlay=$('appDialogOverlay'),dialog=$('appDialog'),title=$('appDialogTitle'),
    desc=$('appDialogDesc'),input=$('appDialogInput'),list=$('appDialogList'),cancelBtn=$('appDialogCancel'),confirmBtn=$('appDialogConfirm');
...
function showFolderPickerDialog(opts={}){
  _ensureAppDialogBindings();
  if(APP_DIALOG.resolve) _finishAppDialog(null,false);
  const overlay=$('appDialogOverlay'),dialog=$('appDialog'),title=$('appDialogTitle'),
    desc=$('appDialogDesc'),input=$('appDialogInput'),list=$('appDialogList'),
    cancelBtn=$('appDialogCancel'),confirmBtn=$('appDialogConfirm');
  APP_DIALOG.resolve=null;APP_DIALOG.kind='folderpicker';APP_DIALOG.lastFocus=document.activeElement;
  APP_DIALOG.selectedValue=null;
  APP_DIALOG.currentPath='';
  if(input) input.style.display='none';
  if(title) title.textContent=opts.title||t('workspace_choose_path');
  if(desc) desc.textContent=opts.message||'';
  ...
  async function _loadFolderList(path){
    currentPath=path;
    APP_DIALOG.currentPath=path;
    if(!list) return;
    list.innerHTML='';
    list.style.display='';

    // Path display (inside list so list.innerHTML='' cleans it up)
    const pathNavEl=document.createElement('div');
    pathNavEl.className='app-dialog-path';
    pathNavEl.textContent=path||'Home';
    list.appendChild(pathNavEl);

    try{
      const qs=path?`prefix=${encodeURIComponent(path)}`:'';
      const data=await api(`/api/workspaces/suggest?${qs}`);
      const suggestions=(data.suggestions||[]);
      const parentPath=data.parentPath;

      // Back button using backend's parentPath
      const hasParent=(parentPath!==undefined && parentPath!=='') || (path!=='' && parentPath===undefined);
      if(hasParent){
        const backBtn=document.createElement('button');
        backBtn.type='button';
        backBtn.className='app-dialog-back-btn';
        backBtn.textContent='← Back';
        backBtn.onclick=()=>_loadFolderList(parentPath||'');
        list.appendChild(backBtn);
      }

      // "Select current folder" option
      const selectCurrent=document.createElement('div');
      selectCurrent.className='app-dialog-list-item';
      selectCurrent.style.borderBottom='1px solid var(--border)';
      const displayPath=path||'Home';
      selectCurrent.innerHTML=`${li('check',14)}<span style="font-weight:500">Select current folder</span><span style="margin-left:auto;font-size:11px;color:var(--muted)">${esc(displayPath)}</span>`;
      selectCurrent.onclick=()=>{
        list.querySelectorAll('.app-dialog-list-item').forEach(i=>i.classList.remove('selected'));
        selectCurrent.classList.add('selected');
        APP_DIALOG.selectedValue=path||'';
      };
      list.appendChild(selectCurrent);

      if(suggestions.length===0){
        const empty=document.createElement('div');
        empty.className='app-dialog-list-empty';
        empty.textContent='No sub-folders found';
        list.appendChild(empty);
      }else{
        for(const sp of suggestions){
          const el=document.createElement('div');
          el.className='app-dialog-list-item';
          const name=_pathName(sp);
          el.innerHTML=`${li('folder',14)}<span>${esc(name)}</span><span style="margin-left:auto;font-size:11px;color:var(--muted);opacity:.7">→</span>`;
          el.onclick=()=>_loadFolderList(sp);
          list.appendChild(el);
        }
      }
    }catch(e){
      const err=document.createElement('div');
      err.className='app-dialog-list-empty';
      err.textContent='Failed to load folders: '+(e&&e.message?e.message:e);
...
function _pickBestVoice(voices, lang){
  ...
  if(exact) return exact;
  const partial=voices.find(v=>v.lang&&v.lang.toLowerCase().includes(langPrefix));
...
function _buildBrowserUtterance(text, btn){
  ...
  if(savedVoice&&voices.length){
    chosenVoice=voices.find(v=>v.name===savedVoice);
  }
  if(!chosenVoice&&voices.length){
    const lang=_detectTextLang(text);
    chosenVoice=_pickBestVoice(voices, lang);
  }
  if(chosenVoice){
    utter.voice=chosenVoice;
    if(chosenVoice.lang) utter.lang=chosenVoice.lang;
  }
...
function _playEdgeTtsChunked(text, btn){
  ...
  const _playOne=function(idx){
    ...
    if(!('speechSynthesis' in window)){
      _ttsSpeaking=false;_playingEdgeAudio=null;
      if(btn) btn.dataset.speaking='0';
      if(typeof showToast==='function') showToast('TTS not supported in this browser');
      return;
    }
    const utter=new SpeechSynthesisUtterance(chunk);
    utter.lang='zh-CN';
    if(!isNaN(savedRate)) utter.rate=savedRate;
    if(!isNaN(savedPitch)) utter.pitch=savedPitch;
    const voices=speechSynthesis.getVoices();
    const voiceName=localStorage.getItem('hermes-tts-voice')||'';
    if(voiceName){
      const match=voices.find(function(v){return v.name.indexOf(voiceName)>=0||v.voiceURI.indexOf(voiceName)>=0;});
...
const INFLIGHT_KEY = 'voldp-ai-agent-inflight'; // localStorage key for in-flight session tracking
const INFLIGHT_STATE_KEY = 'voldp-ai-agent-inflight-state'; // localStorage snapshots for mid-stream reload recovery
...
function restoreLiveTurnHtmlForSession(sid){
  ...
  // existing 可能在 msgInner 中段（被 _preservedLiveTurn 保留在那里），
  // replaceWith 会把 restored 也放到中段。强制移到末尾，确保顺序正确。
  if(inner.lastChild!==restored) inner.appendChild(restored);
...
function setSystemHealthUnavailable(message){
  ...
  if(status) status.textContent=message||t('system_health_unavailable');
...
function renderSystemHealth(payload){
  ...
  if(!payload||payload.available===false){
    setSystemHealthUnavailable(t('system_health_unavailable'));
    return;
  }
  panel.classList.remove('loading','unavailable');
  if(status) status.textContent=payload.status==='partial'?t('system_health_partial'):t('system_health_live');
...
async function showWhatsNewSummary(target){
  ...
  // 调用后端 /api/updates/summary 生成 AI 摘要
  try{
    const resp=await api('/api/updates/summary',{
      method:'POST',
      body:JSON.stringify({updates:data,target:target||''}),
      timeoutMs:60000,
    });
    if(resp&&resp.summary_sections){
      _renderUpdateSummaryPanel(resp,data,target);
    }else{
      _renderUpdateSummaryPanel({summary:(resp&&resp.summary)||'暂无可用更新'},data,target);
    }
  }catch(e){
    _renderUpdateSummaryPanel({summary:'生成摘要失败：'+(e&&e.message||'未知错误')},data,target);
  }
...
async function applyUpdates(){
  showToast('自动更新功能已禁用',5000,'warning');
}
...
function syncTopbar(){
  if(!S.session){
    ...
    // Update work-mode chip label even when no session is active.
    const _profileLabel=$('profileChipLabel');
    if(_profileLabel) {
      const wm = (typeof _getCurrentWorkMode==='function') ? _getCurrentWorkMode() : 'hermes-agent';
      const modeObj = (typeof WORK_MODES!=='undefined') ? WORK_MODES.find(m=>m.id===wm) : null;
      _profileLabel.textContent = (modeObj && typeof _workModeLabel==='function') ? _workModeLabel(modeObj) : ((wm==='claude-code') ? 'Claude Code' : 'Hermes Agent');
    }
    return;
  }
  ...
  // Update work-mode chip label.
  // The composer chip has been repurposed to switch between work modes
  // (Hermes Agent / Claude Code) rather than Hermes profiles.
  // 会话切换冻结期间跳过：避免用旧 S.session 调用 _getCurrentWorkMode 把 chip
  // 写成旧会话模式，造成切换时两个模式反复横跳。由 _syncWorkModeForSession 统一刷新。
  const profileLabel=$('profileChipLabel');
  if(profileLabel && !(typeof _switchingSession!=='undefined' && _switchingSession)) {
    const wm = (typeof _getCurrentWorkMode==='function') ? _getCurrentWorkMode() : 'hermes-agent';
    const modeObj = (typeof WORK_MODES!=='undefined') ? WORK_MODES.find(m=>m.id===wm) : null;
    profileLabel.textContent = (modeObj && typeof _workModeLabel==='function') ? _workModeLabel(modeObj) : ((wm==='claude-code') ? 'Claude Code' : 'Hermes Agent');
  }
}
...
function _toolShortName(name){
  ...
  if(!raw) return 'tool';
  // 内置工具名称翻译（不影响工具调用，仅前端显示）
  const toolNameMap={
    'terminal':'tool_name_terminal',
    'read_file':'tool_name_read_file',
    'write_file':'tool_name_write_file',
    'edit_file':'tool_name_edit_file',
    'search_files':'tool_name_search_files',
    'grep':'tool_name_grep',
    'list_directory':'tool_name_list_directory',
    'python':'tool_name_python',
    'web_fetch':'tool_name_web_fetch',
    'web_search':'tool_name_web_search',
    'api_request':'tool_name_api_request',
    'ask_user':'tool_name_ask_user',
    'save_memory':'tool_name_save_memory',
    'vector_search':'tool_name_vector_search',
    'task_create':'tool_name_task_create',
    'gui_automation':'tool_name_gui_automation',
  };
  const key=toolNameMap[raw.toLowerCase()];
  if(key) return t(key);
  ...
  if(type==='tool'){
    ...
    if(header){
      ...
      if(status==='Completed'){
        ...
        statusEl.textContent=t('tool_preview_'+status.toLowerCase());
...
function _autoCompressionBaseDetail(state){
  ...
  if(running)return (typeof t==='function'?t('compression_running_message'):'Compressing context');
  if(state&&state.phase==='done')return (typeof t==='function'?t('compression_done_message'):'Context auto-compressed');
  return '';
}
...
function appendLiveCompressionCard(state){
  ...
  if(state.automatic){
    ...
    if(state.phase==='running'){
      ...
      node.setAttribute('data-compression-message',String(state.message||(typeof t==='function'?t('compression_running_message'):'Compressing context')));
      _startCompressionElapsedTimer();
    } else {
...
function renderMessages(options){
  ...
  // During active streaming/busy, expand the render window so new turns don't
  // push older context out of view. Cap at 80 messages to avoid unbounded DOM
  // growth and long freezes in very long conversations.
  if((S.busy||S.activeStreamId) && typeof _messageRenderableMessageCount==='function'){
    const STREAM_RENDER_WINDOW_MAX=80;
    _messageRenderWindowSize=Math.min(
      Math.max(_currentMessageRenderWindowSize(), _messageRenderableMessageCount()),
      STREAM_RENDER_WINDOW_MAX
    );
  }
  ...
  if($('emptyState').style.display!=='none'){
    const personality=(S.session&&S.session.personality)?S.session.personality:'helpful';
    if(_emptyStateIntroPersonality!==personality) updateEmptyStateForPersonality(personality);
  }
  ...
  // 放宽保留条件：不再要求 INFLIGHT[sid] 存在。
  // 切换会话时 INFLIGHT 可能在 journalReplayFromStart 或 stale cursor-only 路径
  // 被清理（sessions.js L1149-1162），但 #liveAssistantTurn 的 DOM 节点仍属于
  // 当前会话（dataset.sessionId 匹配），应保留以避免思考/工具UI内容消失。
  // 只在 dataset.sessionId 匹配 sid 时保留，防止跨会话污染。
  {
    const _lt=document.getElementById('liveAssistantTurn');
    if(_lt&&sid&&(!_lt.dataset||!_lt.dataset.sessionId||_lt.dataset.sessionId===sid)){
  ...
  const hasServerOlder=!!(
    typeof _messagesTruncated!=='undefined' && _messagesTruncated && S.messages.length>0 &&
    Number.isFinite(Number(_oldestIdx)) && Number(_oldestIdx)>0
  );
  const serverOlderCount=hasServerOlder?Math.max(0,Number(_oldestIdx)):0;
  if(typeof _applySessionNavigationPrefs==='function') _applySessionNavigationPrefs();
  if(hiddenBeforeCount>0 || hasServerOlder){
    const indicator=document.createElement('button');
    indicator.type='button';
    indicator.id='loadOlderIndicator';
    indicator.className='load-older-indicator message-window-load-earlier'+(typeof _loadingOlder!=='undefined'&&_loadingOlder?' is-loading':'');
    if(typeof _loadingOlder!=='undefined'&&_loadingOlder){
      indicator.textContent='Loading earlier messages...';
    }else{
  ...
  for(let vi=0;vi<renderVisWithIdx.length;vi++){
    ...
    let filesHtml='';
    const msgAttachments=Array.isArray(m.attachments)?m.attachments:(typeof m.attachments==='string'&&m.attachments?[m.attachments]:[]);
    if(msgAttachments.length){
      ...
      filesHtml=`<div class="msg-files">${msgAttachments.map(f=>{
  ...
  // 确保 #liveAssistantTurn 和 #thinkingRow 位于 #msgInner 最末尾。
  // 跨重建保留的占位元素可能停在 msgInner 中段（例如 _preservedLiveTurn
  // 通过 replaceWith 放到了重建 turn 的位置，而重建 turn 不在末尾），
  // 这会导致用户新消息出现在工具/思考UI组件之后，顺序错乱。
  // 无论是否调用 appendThinking()，重建完成后都应纠正位置。
  {
    const _mi=inner;
    if(_mi){
      const _lt=document.getElementById('liveAssistantTurn');
      if(_lt&&_lt.parentNode===_mi&&_mi.lastChild!==_lt){
        _mi.appendChild(_lt);
      }
      const _tr=$('thinkingRow');
      if(_tr&&_tr.parentNode===_mi&&_mi.lastChild!==_tr){
        _mi.appendChild(_tr);
      }
    }
  }
...
function _toolDisplayName(tc){
  ...
  if(name==='subagent_progress') return t('tool_name_subagent');
  if(name==='delegate_task') return t('tool_name_delegate_task');
  if(name==='edit_file') return t('tool_name_edit_file');
  if(name==='list_directory') return t('tool_name_list_directory');
  if(name==='python') return t('tool_name_python');
  if(name==='web_fetch') return t('tool_name_web_fetch');
  if(name==='web_search') return t('tool_name_web_search');
  if(name==='api_request') return t('tool_name_api_request');
  if(name==='ask_user') return t('tool_name_ask_user');
  if(name==='save_memory') return t('tool_name_save_memory');
  if(name==='vector_search') return t('tool_name_vector_search');
  if(name==='task_create') return t('tool_name_task_create');
  if(name==='gui_automation') return t('tool_name_gui_automation');
  if(name==='search_files') return t('tool_name_search_files');
  if(name==='read_file') return t('tool_name_read_file');
  if(name==='grep') return t('tool_name_grep');
  if(name==='write_file') return t('tool_name_write_file');
  if(name==='terminal') return t('tool_name_terminal');
  return name;
}
...
function _toolActionKind(tc){
  ...
  if(n==='python'||n.includes('python')) return 'shell';
  if(n==='web_search'||n.includes('web_search')) return 'web';
  if(n==='api_request'||n.includes('api_request')) return 'web';
  if(n==='ask_user'||n.includes('ask_user')) return 'unknown';
  if(n==='task_create'||n.includes('task_create')) return 'delegate';
  if(n==='gui_automation'||n.includes('gui_automation')) return 'delegate';
  return 'unknown';
}
...
function _toolActionLabelText(tc, opts){
  ...
  const verbs={
    shell:   {ing:t('tool_verb_shell_ing'),   ed:t('tool_verb_shell_ed')},
    read:    {ing:t('tool_verb_read_ing'),    ed:t('tool_verb_read_ed')},
    list:    {ing:t('tool_verb_list_ing'),    ed:t('tool_verb_list_ed')},
    search:  {ing:t('tool_verb_search_ing'), ed:t('tool_verb_search_ed')},
    web:     {ing:t('tool_verb_web_ing'),    ed:t('tool_verb_web_ed')},
    write:   {ing:t('tool_verb_write_ing'),  ed:t('tool_verb_write_ed')},
    delegate:{ing:t('tool_verb_delegate_ing'),ed:t('tool_verb_delegate_ed')},
    unknown: {ing:t('tool_verb_unknown_ing'),ed:t('tool_verb_unknown_ed')},
  };
  const v=verbs[kind]||verbs.unknown;
  const display=_toolDisplayName(tc);
  if(isErr){
    const errAction=target?`${v.ing.toLowerCase()} ${target}`:`${v.ing.toLowerCase()} ${display}`;
    return t('tool_failed_prefix',errAction);
  }
...
function _getToolWorklogSummaries(){
  return {
    shell:{running:t('tool_wl_shell_running'),runningMany:t('tool_wl_shell_running_many'),done:t('tool_wl_shell_done'),doneMany:t('tool_wl_shell_done_many')},
    read:{running:t('tool_wl_read_running'),runningMany:t('tool_wl_read_running_many'),done:t('tool_wl_read_done'),doneMany:t('tool_wl_read_done_many')},
    list:{running:t('tool_wl_list_running'),runningMany:t('tool_wl_list_running_many'),done:t('tool_wl_list_done'),doneMany:t('tool_wl_list_done_many')},
    search:{running:t('tool_wl_search_running'),runningMany:t('tool_wl_search_running_many'),done:t('tool_wl_search_done'),doneMany:t('tool_wl_search_done_many')},
    web:{running:t('tool_wl_web_running'),runningMany:t('tool_wl_web_running_many'),done:t('tool_wl_web_done'),doneMany:t('tool_wl_web_done_many')},
    write:{running:t('tool_wl_write_running'),runningMany:t('tool_wl_write_running_many'),done:t('tool_wl_write_done'),doneMany:t('tool_wl_write_done_many')},
    delegate:{running:t('tool_wl_delegate_running'),runningMany:t('tool_wl_delegate_running_many'),done:t('tool_wl_delegate_done'),doneMany:t('tool_wl_delegate_done_many')},
    unknown:{running:t('tool_wl_unknown_running'),runningMany:t('tool_wl_unknown_running_many'),done:t('tool_wl_unknown_done'),doneMany:t('tool_wl_unknown_done_many')},
  };
}
function _toolWorklogActionParts(tc){
  const summaries=_getToolWorklogSummaries();
  if(tc&&tc.nodeType===1){
    ...
    return {kind,isDone,isErr,target:'',summary:summaries[kind]||summaries.unknown,actionLabel};
  }
  const kind=_toolActionKind(tc);
  return {
    kind,
    ...
    summary:summaries[kind]||summaries.unknown,
...
function _toolWorklogSummary(toolCalls, opts){
  ...
  if(!cards.length) return (opts&&opts.live)?t('tool_worklog_running'):t('tool_worklog_default');
  if(cards.length===1){
    const part=_toolWorklogActionParts(cards[0]);
    const fmt=part.summary||summaries.unknown;
    const line=part.isDone?fmt.done:fmt.running;
    return part.isErr?t('tool_worklog_one_failed',line):line;
  }
  ...
  const emit=(counts,state)=>{
    ...
    for(const kind of order){
      ...
      if(!n) continue;
      const fmt=summaries[kind]||summaries.unknown;
      if(n===1) out.push(state==='done'?fmt.done:fmt.running);
      else out.push((state==='done'?fmt.doneMany:fmt.runningMany).replace('{0}',String(n)));
    }
    return out;
  };
  const lines=[...emit(runningCounts,'running'),...emit(doneCounts,'done')];
  if(failed) lines.push(t('tool_worklog_n_failed',failed));
...
function _syncToolRowsContainer(tools, isLiveWorklog){
  ...
  const summary=hasRunning?t('tool_worklog_running'):_toolWorklogSummary(rows,{live:isLiveWorklog, toolCount:rows.length});
...
function toolIcon(name){
  ...
  const icons={
    ...
    edit_file:       li('file-diff'),
    search_files:    li('search'),
    grep:            li('search-code'),
    list_directory:  li('folder-open'),
    web_search:      li('globe'),
    web_extract:     li('globe'),
    web_fetch:       li('globe'),
    execute_code:    li('play'),
    python:          li('code-2'),
    api_request:     li('webhook'),
    ask_user:        li('help-circle'),
    save_memory:     li('database'),
    vector_search:   li('brain-circuit'),
    task_create:     li('list-start'),
    gui_automation:  li('monitor'),
    patch:           li('wrench'),
    memory:          li('brain'),
...
function _toolCardPreviewText(tc, displaySnippet){
  ...
  if(tc&&tc.done===false) return t('tool_preview_running');
  if(tc&&tc.is_error) return t('tool_preview_failed');
  return t('tool_preview_completed');
}
function buildToolCard(tc){
  ...
  const moreLabel=tc.is_diff?t('tool_show_diff'):t('tool_show_more');
  const lessLabel=tc.is_diff?t('tool_hide_diff'):t('tool_show_less');
...
function _syncToolCallGroupSummary(group){
  ...
  if(label){
    if(group.getAttribute('data-run-activity-group')==='1'){
      label.textContent=toolCount?_toolWorklogSummary(cards,{live:isLiveWorklog, toolCount}):t('tool_worklog_running');
      ...
      if(!label.textContent) label.textContent=isLiveWorklog?t('tool_worklog_running'):t('tool_worklog_default');
    }else{
      ...
      let suffix='';
      if(memCount) suffix+=`, ${memCount===1?t('tool_activity_memory_one'):t('tool_activity_memory_many',memCount)}`;
      if(skillCount) suffix+=`, ${skillCount===1?t('tool_activity_skill_one'):t('tool_activity_skill_many',skillCount)}`;
      const toolsPart=otherCount?(otherCount===1?t('tool_activity_tools_one'):t('tool_activity_tools_many',otherCount)):'';
      if(group.getAttribute('data-live-tool-call-group')==='1'){
        if(toolsPart) label.textContent=t('tool_activity_format',toolsPart+suffix);
        else if(suffix) label.textContent=t('tool_activity_format',suffix.slice(2));
        else label.textContent=t('tool_worklog_running');
      }else if(toolsPart||suffix){
        label.textContent=toolsPart?t('tool_activity_format',toolsPart+suffix):t('tool_activity_format',suffix.slice(2));
      }else label.textContent=t('tool_activity');
    }
...
function _activityProgressLabelForToolName(name){
  ...
  if(!key) return t('tool_progress_default');
  if(key.includes('search')||key.includes('grep')) return t('tool_progress_search');
  if(key.includes('read')||key.includes('view')||key.includes('open')) return t('tool_progress_read');
  if(key.includes('write')||key.includes('patch')||key.includes('edit')) return t('tool_progress_write');
  if(key.includes('terminal')||key.includes('shell')||key.includes('command')||key.includes('process')) return t('tool_progress_shell');
  if(key.includes('web')||key.includes('fetch')||key.includes('curl')) return t('tool_progress_web');
  if(key.includes('todo')||key.includes('plan')) return t('tool_progress_todo');
  if(key.includes('python')) return t('tool_progress_python');
  if(key.includes('web_search')) return t('tool_progress_web_search');
  if(key.includes('api_request')) return t('tool_progress_api_request');
  if(key.includes('ask_user')) return t('tool_progress_ask_user');
  if(key.includes('save_memory')) return t('tool_progress_save_memory');
  if(key.includes('vector_search')) return t('tool_progress_vector_search');
  if(key.includes('task_create')) return t('tool_progress_task_create');
  if(key.includes('gui_automation')) return t('tool_progress_gui_automation');
  return t('tool_progress_default');
}
...
function postProcessRenderedMessages(container) {
  // 非流式/非繁忙时把重渲染后的高亮、公式、图表等耗时后处理放到空闲时段执行，
  // 避免在主线程阻塞用户交互；流式期间仍需立即处理，确保代码块等及时可见。
  if(!S.busy && !S.activeStreamId && typeof requestIdleCallback==='function'){
    requestIdleCallback(()=>_postProcessRenderedMessages(container),{timeout:300});
  }else{
    _postProcessRenderedMessages(container);
  }
}
function _postProcessRenderedMessages(container) {
...
function loadDiffInline(container){
  ...
  root.querySelectorAll('.diff-inline-load:not([data-loaded])').forEach(el=>{
    ...
    fetch(new URL('api/media?path='+encodeURIComponent(path),document.baseURI||location.href).href,{credentials:'include'})
...
function _thinkingMarkup(text=''){
  ...
  // apiRetry indicator
  if(_apiRetryState){
    const statusBadge=_apiRetryState.errorStatus?`[${_apiRetryState.errorStatus}] `:_apiRetryState.errorType?`[${_apiRetryState.errorType}] `:'[Error] ';
    const retryLabel=_apiRetryState.retryCount>1?` (#${_apiRetryState.retryCount})`:_apiRetryState.retryCount===1?' (1st retry)':'';
    return `<div class="streaming-indicator streaming-indicator-retry" id="streamingIndicator"><span class="streaming-indicator-icon">↻</span><span class="streaming-indicator-verb">${esc(statusBadge)}${t('streaming_retrying')||'Retrying'}${retryLabel}…</span><span class="streaming-indicator-retry-countdown" id="streamingRetryCountdown"></span><span class="streaming-indicator-retry-detail">${esc(_apiRetryState.message||'')}</span></div>`;
  }
  // streamingFallback indicator
  if(_streamingFallbackState){
    return `<div class="streaming-indicator streaming-indicator-fallback" id="streamingIndicator"><span class="streaming-indicator-icon">⧗</span><span class="streaming-indicator-verb">${esc(_streamingFallbackState.message)}</span></div>`;
  }
  // default loading indicator
  return `<div class="streaming-indicator" id="streamingIndicator"><span class="streaming-indicator-icon">✦</span><span class="streaming-indicator-verb">${t('streaming_thinking')||'Thinking'}...</span><span class="streaming-indicator-elapsed" id="streamingElapsed"></span><span class="streaming-indicator-tokens" id="streamingTokens"></span></div>`;
}
function _renderThinkingInto(row,text=''){
  if(!row) return;
  const clean=_sanitizeThinkingDisplayText(text);
  if(!clean){
    row.classList.add('thinking-row-streaming');
    row.innerHTML=_thinkingMarkup(text);
    // 确保 streaming-indicator 与 composer-box 水平对齐
    if(typeof _syncComposerCenterVar==='function') _syncComposerCenterVar();
    return;
  }
  row.classList.remove('thinking-row-streaming');
  const pre=row.querySelector('.thinking-card-body pre');
  if(pre){
...
function finalizeThinkingCard(){
  // 重置节流缓存，避免下一次流式传输时缓存干扰
  _lastThinkingText='';
  _appendThinkingPending=null;
  _appendThinkingScheduled=false;
...
function appendThinking(text='', options){
  ...
  // 节流：缓存最新文本，使用requestAnimationFrame限制DOM更新频率
  _appendThinkingPending={text,options};
  if(_appendThinkingScheduled) return;
  _appendThinkingScheduled=true;
  requestAnimationFrame(()=>{
    _appendThinkingScheduled=false;
    if(!_appendThinkingPending) return;
    const {text:pText,options:pOptions}=_appendThinkingPending;
    _appendThinkingPending=null;
    _appendThinkingFlush(pText,pOptions);
  });
}

let _appendThinkingScheduled=false;
let _appendThinkingPending=null;
let _lastThinkingText=''; // 缓存上一次渲染的文本，避免重复DOM更新

function _appendThinkingFlush(text='', options){
  options=options||{};
  const allowPendingPlaceholder=!!(options&&options.pending===true);
  if(!S.session||(!S.activeStreamId&&!allowPendingPlaceholder)) return;
  const empty=$('emptyState');
  if(empty) empty.style.display='none';
  // 切换会话后，#liveAssistantTurn 和 #thinkingRow 可能被 _preservedLiveTurn
  // 保留在 msgInner 的中间位置（而非末尾），导致"思考中"指示器和工具UI组件
  // 显示在历史消息之上。这里统一将它们移到 msgInner 最末尾，确保顺序正确：
  // 历史消息 → liveAssistantTurn（工具卡片）→ thinkingRow（思考指示器）。
  {
    const _mi=$('msgInner');
    if(_mi){
      const _lt=document.getElementById('liveAssistantTurn');
  ...
  if(clean&&window._showThinking!==false){
    ...
    if(isTransparentStream()){
      ...
      if(!row){
        ...
        // 缓存检查：如果文本没有变化，跳过DOM更新
        if(clean===_lastThinkingText) return;
        _lastThinkingText=clean;
        const existingText=row.querySelector('.thinking-card-body pre')?.textContent||'';
        if(existingText!==clean) _renderThinkingInto(row, clean);
      }
      row.id='thinkingRow';
      row.setAttribute('data-thinking-active','1');
  ...
  // When clean is empty but we're in a pending/streaming state, show the default
  // streaming indicator (spinner / retry / fallback) so the user sees feedback.
  if(!clean&&(allowPendingPlaceholder||S.activeStreamId)){
    let row=blocks.querySelector('#thinkingRow');
    if(!row){
      row=document.createElement('div');
      row.id='thinkingRow';
      row.className='thinking-card-row';
...
function removeThinking(){
  ...
  if(blocks) blocks.querySelectorAll('#thinkingRow').forEach(el=>el.remove());
  ...
  if(blocks) blocks.querySelectorAll('.live-worklog[data-live-worklog-shell="1"],.tool-worklog-group[data-live-tool-call-group="1"],.tool-call-group[data-live-tool-call-group="1"],.tool-call-group[data-agent-activity-group="1"]').forEach(group=>{
    _syncToolCallGroupSummary(group);
    if(!group.querySelector('.tool-card-row,.agent-activity-thinking,.wl-reason,#thinkingRow')){
      if(typeof _clearActivityElapsedTimer==='function') _clearActivityElapsedTimer();
      group.remove();
    }
  });
...
function _syncWorkspaceHeadingState(){
  ...
  if(enabled){
    ...
    heading.title=t('workspace_root');
  } else {
...
function _showWorkspaceRootContextMenu(e){
  ...
  menu.style.cssText='position:fixed;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:6px 0;z-index:9999;min-width:160px;box-shadow:0 4px 16px rgba(0,0,0,.35);visibility:hidden;';
  ...
  // Paste（仅当剪贴板非空时显示，粘贴到工作区根目录）
  if(S._fileClipboard){
    const pasteSep=document.createElement('hr');
    pasteSep.style.cssText='border:none;border-top:1px solid var(--border);margin:4px 0;';
    menu.appendChild(pasteSep);
    menu.appendChild(_workspaceContextMenuItem(t('paste_title'),async()=>{
      menu.remove();
      await _performWorkspacePaste('.');
    }));
  }
  ...
  document.body.appendChild(menu);
  _keepMenuInViewport(menu,e);
  const dismiss=()=>{menu.remove();document.removeEventListener('click',dismiss);};
  setTimeout(()=>document.addEventListener('click',dismiss),0);
}

function _keepMenuInViewport(menu,e){
...
// File clipboard for copy/cut/paste: { path, mode, isDir }
// mode: 'copy' | 'cut'
if(!S._fileClipboard) S._fileClipboard=null;
...
// 执行粘贴操作：复制或移动剪贴板中的文件/目录到目标目录
async function _performWorkspacePaste(destDir){
  if(!S.session||!S._fileClipboard)return;
  const clip=S._fileClipboard;
  const normDest=destDir||'.';
  // 不能粘贴到自身
  if(clip.path===normDest){
    showToast(t('paste_same_dir')||'Cannot paste into itself',3000,'error');
    return;
  }
  // 不能粘贴到自己的子目录（避免递归）
  if(normDest.startsWith(clip.path+'/')){
    showToast(t('paste_into_subdir')||'Cannot paste into a subdirectory of the source',3000,'error');
    return;
  }
  // 剪切模式下源和目标父目录相同时无需操作（复制模式允许在同目录生成副本）
  if(clip.mode==='cut'&&_workspaceParentDir(clip.path)===normDest){
    showToast(t('paste_same_location')||'Already in this location',3000);
    return;
  }
  try{
    if(clip.mode==='copy'){
      const data=await api('/api/file/copy',{method:'POST',body:JSON.stringify({
        session_id:S.session.session_id,path:clip.path,dest_dir:normDest
      })});
      const copiedName=data.new_path.includes('/')?data.new_path.slice(data.new_path.lastIndexOf('/')+1):data.new_path;
      showToast((t('copied_to')||'Copied to ')+copiedName);
    }else{
      // cut: 移动文件
      const data=await api('/api/file/move',{method:'POST',body:JSON.stringify({
        session_id:S.session.session_id,path:clip.path,dest_dir:normDest
      ...
      const movedName=data.new_path.includes('/')?data.new_path.slice(data.new_path.lastIndexOf('/')+1):data.new_path;
      showToast((t('moved_to')||'Moved to ')+movedName);
      _remapWorkspaceCachesAfterMove(data.old_path||clip.path,data.new_path||clip.path,clip.isDir);
      // 剪切粘贴后清空剪贴板
      S._fileClipboard=null;
    }
    // 刷新目录
    delete S._dirCache[normDest];
    await loadDir(S.currentDir);
    if(typeof refreshOpenPreviewIfMutated==='function')await refreshOpenPreviewIfMutated();
  }catch(err){
    showToast((t('paste_failed')||'Paste failed: ')+err.message,5000,'error');
  }
}
...
function _showFileContextMenu(e, item){
  ...
  menu.style.cssText='position:fixed;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:6px 0;z-index:9999;min-width:140px;box-shadow:0 4px 16px rgba(0,0,0,.35);visibility:hidden;';
  ...
  // Copy（复制当前项到剪贴板）
  menu.appendChild(_workspaceContextMenuItem(t('copy_title'),()=>{
    menu.remove();
    S._fileClipboard={path:item.path,mode:'copy',isDir:item.type==='dir'};
    showToast(t('copy_clipboard')||'Copied to clipboard');
  }));

  // Cut（剪切当前项到剪贴板）
  menu.appendChild(_workspaceContextMenuItem(t('cut_title'),()=>{
    menu.remove();
    S._fileClipboard={path:item.path,mode:'cut',isDir:item.type==='dir'};
    showToast(t('cut_clipboard')||'Cut to clipboard');
  }));

  // Paste（仅当剪贴板非空时显示，粘贴到当前目录或文件夹）
  if(S._fileClipboard){
    const pasteDest=item.type==='dir'?item.path:targetDir;
    menu.appendChild(_workspaceContextMenuItem(t('paste_title'),async()=>{
      menu.remove();
      await _performWorkspacePaste(pasteDest);
    }));
  }
  ...
  if(item.type==='dir'){
    ...
    dlItem.onclick=()=>{
      menu.remove();
      // 使用 new URL(baseURI) 包装，确保子路径挂载下也能正确解析（与项目内其他下载调用一致）
    ...
    // 单文件下载：复用 /api/file/raw?download=1 端点，强制浏览器下载而非内联显示
    const dlItem=document.createElement('div');
    dlItem.textContent=t('download_file');
    dlItem.style.cssText='padding:7px 14px;cursor:pointer;font-size:13px;color:var(--text);';
    dlItem.onmouseenter=()=>dlItem.style.background='var(--hover-bg)';
    dlItem.onmouseleave=()=>dlItem.style.background='';
    dlItem.onclick=()=>{
      menu.remove();
      const url=new URL('api/file/raw?session_id='+encodeURIComponent(S.session.session_id)
              + '&path='+encodeURIComponent(item.path||'')+'&download=1', document.baseURI||location.href).href;
      window.location.href=url;
    };
    menu.appendChild(dlItem);
  }
...
function _uploadTooLargeMessage(file,archive){
  const fileSizeMb=Math.ceil(((file&&file.size)||0)/1024/1024);
  const maxMb=archive?MAX_ARCHIVE_UPLOAD_MB:MAX_UPLOAD_MB;
  return t('upload_too_large',maxMb,fileSizeMb);
}
function _showUploadTooLarge(file,archive){
  const message=`${t('upload_failed')}${file&&file.name?file.name:'file'} \u2014 ${_uploadTooLargeMessage(file,archive)}`;
  if(typeof setStatus==='function')setStatus(`\u274c ${message}`);
  else if(typeof showToast==='function')showToast(message,5000,'error');
}
function addFiles(files){
  for(const f of files){
    const archive=_ARCHIVE_EXTS.test(f.name||'');
    const limit=archive?MAX_ARCHIVE_UPLOAD_BYTES:MAX_UPLOAD_BYTES;
    if(f&&f.size>limit){_showUploadTooLarge(f,archive);continue;}
...
async function uploadPendingFiles(){
  ...
  for(let i=0;i<total;i++){
    ...
    try{
      ...
      if(f&&f.size>(isArchive?MAX_ARCHIVE_UPLOAD_BYTES:MAX_UPLOAD_BYTES))throw new Error(_uploadTooLargeMessage(f,isArchive));
      const fd=new FormData();
      fd.append('session_id',S.session.session_id);fd.append('file',f,f.name);
      const url=new URL(isArchive?'api/upload/extract':'api/upload',document.baseURI||location.href).href;
      // 实时字节进度：当前文件的进度映射到整体进度条
      // baseProgress 是之前已完成文件的进度起点，fileSpan 是当前文件占整体的百分比
      const baseProgress=(i/total)*100;
      const fileSpan=100/total;
      const res=await uploadWithProgress(url,fd,(loaded,totalBytes)=>{
        const filePct=totalBytes>0?(loaded/totalBytes)*fileSpan:0;
        bar.style.width=`${Math.round(baseProgress+filePct)}%`;
      });
      // 401 重定向到登录页
      if(res.status===401){if(typeof _redirectIfUnauth==='function')_redirectIfUnauth({status:401});return;}
      if(!res.ok){
        const errText=(res.data&&res.data.error)?res.data.error:(res.text&&res.text.trim()?res.text.trim():(res.statusText||`HTTP ${res.status}`));
