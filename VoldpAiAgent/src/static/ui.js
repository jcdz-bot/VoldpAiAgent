...
async function populateModelDropdown(opts={}){
  console.log('[webui-debug] populateModelDropdown called, opts:', JSON.stringify(opts));
  const sel=$('modelSelect');
  if(!sel){console.log('[webui-debug] populateModelDropdown: modelSelect not found'); return;}
  try{
    ...
    console.log('[webui-debug] populateModelDropdown groups count:', groups.length, 'active_provider:', data.active_provider);
...
function _mergeUsageForCtxIndicator(latest, fallback){
  ...
  for(const field of ['input_tokens','output_tokens','estimated_cost']){
...
function showFolderPickerDialog(opts={}){
  console.log('[debug] showFolderPickerDialog called, items:', opts.items);
  _ensureAppDialogBindings();
  if(APP_DIALOG.resolve) _finishAppDialog(null,false);
  const overlay=$('appDialogOverlay'),dialog=$('appDialog'),title=$('appDialogTitle'),
  ...
  if(overlay){
    ...
    console.log('[debug] overlay displayed');
  }else{
    console.log('[debug] overlay NOT found');
  }
  return new Promise(resolve=>{
    APP_DIALOG.resolve=resolve;
...
async function showWhatsNewSummary(target){
  ...
  _renderUpdateSummaryPanel({summary:'自动更新功能已禁用'},data,target);
...
function syncTopbar(){
  if(!S.session){
    ...
    if(_profileLabel) {
      let workModeLabel = (typeof t === 'function' && t('work_mode_hermes_agent')) || 'Hermes Agent';
      try {
        const saved = localStorage.getItem('hermes_web_work_mode');
        if (saved === 'claude-code') workModeLabel = (typeof t === 'function' && t('work_mode_claude_code')) || 'Claude Code';
      } catch (_) {}
      _profileLabel.textContent = workModeLabel;
    }
    return;
  }
  ...
  if(profileLabel) {
    ...
    profileLabel.textContent = workModeLabel;
  }
}
  ...
  for(let i=0;i<total;i++){
    ...
    try{
      ...
      if(!res.ok){
        ...
        throw new Error(err&&err.trim()?err.trim():statusText);
      }
