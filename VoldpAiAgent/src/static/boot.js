...
(function(){
  ...
  try{localStorage.removeItem('voldp-ai-agent-server-stopped');}catch(_){}
  // Listen for shutdown broadcast from other tabs
  try {
    var _stopChan = new BroadcastChannel('voldp-ai-agent-shutdown');
...
// SSE-triggered notification sounds happen outside user gestures, so we pre-create
// a shared AudioContext on the first pointer/touch/keydown interaction and reuse
// it for all subsequent notification/attention sounds.
window._sharedAudioContext=null;
window._audioUnlockAttempted=false;
window._getSharedAudioContext=function(){
  if(window._sharedAudioContext&&window._sharedAudioContext.state!=='closed')return window._sharedAudioContext;
  try{
    const Ctx=window.AudioContext||window.webkitAudioContext;
    if(!Ctx)return null;
    window._sharedAudioContext=new Ctx();
  }catch(_){return null;}
  return window._sharedAudioContext;
};
function _unlockAudio(){
  if(window._audioUnlockAttempted)return;
  window._audioUnlockAttempted=true;
  try{
    const ctx=window._getSharedAudioContext();
    if(ctx&&ctx.state==='suspended')ctx.resume();
  }catch(_){}
}
window.addEventListener('pointerdown',_unlockAudio,{once:true,passive:true});
window.addEventListener('touchend',_unlockAudio,{once:true,passive:true});
window.addEventListener('keydown',_unlockAudio,{once:true,passive:true});

// ── Global SSE event stream ───────────────────────────────────────────────
// Connects to /api/events/stream to receive server-pushed events like
// skills_reloaded (hot-reload notification from the skill change detector).
(function(){
  if(typeof EventSource==='undefined') return;
  try {
    var url = new URL('api/events/stream', document.baseURI || location.href).href;
    var es = new EventSource(url, {withCredentials:true});
    es.addEventListener('message', function(ev) {
      try {
        var msg = JSON.parse(ev.data);
        if (!msg || !msg.type) return;
        // skills_reloaded: 技能热重载通知，刷新前端技能缓存
        if (msg.type === 'skills_reloaded') {
          ...
          if (typeof showToast === 'function') {
            var names = (msg.data && msg.data.changed_skills) || [];
            showToast('Skills hot-reloaded: ' + names.join(', '));
          }
        }
        // workspace_changed: 工作区文件变更通知，自动刷新文件树
        // 后端轮询检测到工作区文件变更后推送，前端比对 workspace 路径后刷新
        if (msg.type === 'workspace_changed') {
          var changedWS = msg.data && msg.data.workspace;
          // 仅刷新当前活动会话的工作区（路径匹配），切换会话冻结期间跳过
          if (changedWS && typeof S !== 'undefined' && S.session && S.session.workspace === changedWS
              && !(typeof _switchingSession !== 'undefined' && _switchingSession)) {
            if (typeof loadDir === 'function' && S.currentDir) {
              // 保留预览状态，只刷新文件树（含展开目录）
              loadDir(S.currentDir, {refreshExpanded: true, preservePreview: true});
            }
          }
        }
      } catch(_) {}
...
function _setWorkspacePanelMode(mode){
  ...
  try{localStorage.setItem('voldp-ai-agent-workspace-panel', open ? 'open' : 'closed');}catch(_){}
  ...
  // 工作区面板展开/收起会改变 main 区域宽度，需同步 composer 中心位置变量。
  // 立即更新一次，然后在 CSS transition（width .24s）结束后再更新一次，
  // 确保 streaming-indicator 与 textarea 水平对齐。
  if(typeof _syncComposerCenterVar==='function') _syncComposerCenterVar();
  setTimeout(()=>{if(typeof _syncComposerCenterVar==='function') _syncComposerCenterVar();},280);
}
...
const _SIDEBAR_COLLAPSED_KEY='voldp-ai-agent-sidebar-collapsed';
...
(function(){
  ...
  let _wavRecorder=null;
  let _wavAudioContext=null;
  let _wavStream=null;
  ...
  async function _startWavRecording(){
    _wavStream=await navigator.mediaDevices.getUserMedia({audio:true});
    const audioContext=new (window.AudioContext||window.webkitAudioContext)({sampleRate:16000});
    _wavAudioContext=audioContext;
    const source=audioContext.createMediaStreamSource(_wavStream);
    ...
    _wavRecorder={
      stop:function(){
        ...
        source.disconnect();
        if(_wavStream){
          _wavStream.getTracks().forEach(function(t){t.stop();});
          _wavStream=null;
        }
        if(_wavAudioContext && _wavAudioContext.state!=='closed'){
          _wavAudioContext.close();
          _wavAudioContext=null;
        }
        const length=chunks.reduce(function(sum,c){return sum+c.length;},0);
        const buffer=new Float32Array(length);
        let offset=0;
        chunks.forEach(function(c){buffer.set(c,offset);offset+=c.length;});
        const wavBlob=_encodeWAV(buffer,audioContext.sampleRate);
        return wavBlob;
      }
    };
  }
  ...
  function _stopWavRecording(){
    if(!_wavRecorder) return;
    const blob=_wavRecorder.stop();
    _wavRecorder=null;
    _isRecording=false;
    _setRecording(false);
    if(blob && blob.size){
  ...
  async function _sendRawAudio(blob){
    let ext='webm';
    if(blob.type&&blob.type.includes('ogg')) ext='ogg';
    else if(blob.type&&blob.type.includes('wav')) ext='wav';
  ...
  async function _transcribeBlob(blob){
    ...
    try{
      const res=await fetch(new URL('api/transcribe',document.baseURI||location.href).href,{method:'POST',body:form,credentials:'include'});
  ...
  function _stopMic(){
    ...
    if(_wavRecorder && _activeCaptureMode==='wav-raw'){
      _stopWavRecording();
      return;
    }
  ...
  async function _probeServerSttCapability(){
    ...
    try{
      const res=await fetch(new URL('api/transcribe/capability',document.baseURI||location.href).href,{cache:'no-store',credentials:'include'});
      const data=await res.json().catch(()=>({}));
  ...
  btn.onclick=async()=>{
    ...
    try{
      if(_rawAudioMode){
        await _startWavRecording();
        _activeCaptureMode='wav-raw';
        _setRecording(true);
        return;
      }
...
(function(){
  ...
  function _speakResponse(){
    ...
    if(engine==="edge"){
      // Edge TTS 后端暂不支持，回退到浏览器 speechSynthesis
      _ttsSpeaking=true;
      ...
      const edgeUtter=new SpeechSynthesisUtterance(clean);
      edgeUtter.lang='zh-CN';
      const savedRateE=parseFloat(localStorage.getItem('hermes-tts-rate'));
      if(!isNaN(savedRateE)) edgeUtter.rate=Math.min(2,Math.max(0.5,savedRateE));
      const savedPitchE=parseFloat(localStorage.getItem('hermes-tts-pitch'));
      if(!isNaN(savedPitchE)) edgeUtter.pitch=Math.min(2,Math.max(0,savedPitchE));
      const edgeVoices=speechSynthesis.getVoices();
      const edgeVoiceName=localStorage.getItem('hermes-tts-voice')||'';
      if(edgeVoiceName&&edgeVoices.length){
        const edgeMatch=edgeVoices.find(v=>v.name===edgeVoiceName||v.name.indexOf(edgeVoiceName)>=0);
        if(edgeMatch) edgeUtter.voice=edgeMatch;
      }else{
        const zhV=edgeVoices.find(v=>v.lang&&v.lang.indexOf('zh')>=0);
        if(zhV) edgeUtter.voice=zhV;
      }
      edgeUtter.onend=()=>{
        _ttsSpeaking=false;
      ...
      speechSynthesis.speak(edgeUtter);
      return;
    }
...
$('btnExportJSON').onclick=()=>{
  if(!S.session)return;
  // 使用 new URL(baseURI) 包装，确保子路径挂载下也能正确解析
...
function _applySessionContextMetadataUpdate(data){
  ...
  S.session.compression_count=data.session.compression_count||0;
...
$('modelSelect').onchange=async()=>{
  ...
  else try{localStorage.setItem('voldp-ai-agent-model',modelState.model)}catch{}
  if(!S.session){
...
document.querySelectorAll('.suggestion').forEach(btn=>{
  btn.onclick=()=>{
    ...
    $('msg').value=span?span.textContent.trim():btn.dataset.msg;
    send();
  };
});
...
(async()=>{
  ...
  try{
    ...
    window._notificationsEnabled=s.notifications_enabled!==false;
    ...
    window._worklogDetailsExpandedByDefault=!!(
      Object.prototype.hasOwnProperty.call(s,'worklog_details_expanded_default')
        ...
        : Object.prototype.hasOwnProperty.call(s,'activity_feed_expanded_default')
    ...
    window._sessionEndlessScrollEnabled=s.session_endless_scroll===true;
    ...
    // 自动请求浏览器通知权限（默认勾选时）
    if(window._notificationsEnabled && 'Notification' in window && Notification.permission === 'default'){
      try{ Notification.requestPermission().then(function(p){ if(typeof updateNotificationPermissionStatus==='function') updateNotificationPermissionStatus(); }); }catch(_){}
    }
  ...
  applyBotName();
  // Update profile chip label to show the current work mode
  ...
  const _hydrateBootModelDropdown=()=>populateModelDropdown({preferProfileDefaultOnFreshBoot:true}).then(()=>{
    ...
    const savedState=(typeof _readPersistedModelState==='function')
      ...
      : (localStorage.getItem('voldp-ai-agent-model')?{model:localStorage.getItem('voldp-ai-agent-model'),model_provider:null}:null);
    ...
    if(savedModel && $('modelSelect')){
      ...
      if(!applied&&sessionModelState&&typeof _ensureModelOptionInDropdown==='function'){
        ...
        if(typeof _clearPersistedModelState==='function') _clearPersistedModelState();
          ...
          localStorage.removeItem('voldp-ai-agent-model-state');
        }
      }
  ...
  const savedLocal=localStorage.getItem('voldp-ai-agent-session');
  ...
  if(saved){
    try{
      ...
      if(S.session && (S.session.message_count||0) === 0 && !_restoredInFlight && !_restoredHasDraft){
        ...
        const _ephPanelPref=localStorage.getItem('voldp-ai-agent-workspace-panel-pref')==='open'
          || localStorage.getItem('voldp-ai-agent-workspace-panel')==='open';
      ...
      const panelPref=localStorage.getItem('voldp-ai-agent-workspace-panel-pref')==='open'
        || localStorage.getItem('voldp-ai-agent-workspace-panel')==='open';
    ...
    catch(e){localStorage.removeItem('voldp-ai-agent-session');}
  }
  ...
  const _freshPanelPref=localStorage.getItem('voldp-ai-agent-workspace-panel-pref')==='open'
    || localStorage.getItem('voldp-ai-agent-workspace-panel')==='open';
...
window.addEventListener('pageshow', async (event) => {
  ...
  if (typeof _isSidebarCollapsed === 'function' && typeof toggleSidebar === 'function') {
    try {
      const _want = localStorage.getItem('voldp-ai-agent-sidebar-collapsed') === '1';
...
async function shutdownServer() {
  const ok = await showConfirmDialog({
    title: (typeof t === 'function' ? t('settings_shutdown_confirm_title') : 'Stop VoldpAiAgent'),
    message: (typeof t === 'function' ? t('settings_shutdown_confirm_message') : 'Stop the VoldpAiAgent server?'),
    confirmLabel: (typeof t === 'function' ? t('settings_shutdown_confirm_btn') : 'Stop'),
    danger: true,
  });
  if (!ok) return;
  // 独立模式下后端不支持远程关机，仅提示用户手动关闭
  try { await api('/api/shutdown', { method: 'POST' }); } catch (_) {}
  if(typeof showToast==='function'){
    showToast('独立模式不支持远程关机，请手动关闭服务器进程', 5000, 'error');
  } else {
    alert('独立模式不支持远程关机，请手动关闭服务器进程');
  }
}
